...
#define   EV_Agua_on        output_bit(PIN_B2,0)
#define   EV_Agua_off       output_bit(PIN_B2,1)

#define   EV_Aire_on        output_bit(PIN_B3,0)
#define   EV_Aire_off       output_bit(PIN_B3,1)

#define   EV_Vacio_on       output_bit(PIN_B4,0)
#define   EV_Vacio_off      output_bit(PIN_B4,1)

#define   EV_AireBomba_on   output_bit(PIN_B5,0)
#define   EV_AireBomba_off  output_bit(PIN_B5,1)

#define   SSR2_on           output_bit(PIN_C2,0)
#define   SSR2_off          output_bit(PIN_C2,1)

#define   SSR3_on           output_bit(PIN_C6,0)
#define   SSR3_off          output_bit(PIN_C6,1)
...
//Hardware//_----------------------
int8 EVVacio=0,EVAire=0,EVAgua=0,SSR2=0,SSR3=0,Buzzer=0,EVAireBomba=0;
...
int8 Resistencia=0,EVDesfogue=0,EVTrampa=0,MotorPuerta=0,EVEntradaAgua=0,BombaVacio=0,BombaAgua=0,Control=0,Alarma=0,AlarmaAgua=0;
...
if(usb_enumerated())
{
   ...
   if(usb_kbhit(1))
   {
    ...
    d3=d3*10.0;
    recbuf[2]=(int8)d3;
    recbuf[3]=(int8)rxbuf[2];
    recbuf[4]=(int8)rxbuf[3];
    recbuf[5]=(int8)rxbuf[4];    
    recbuf[6]=(int8)rxbuf[5];
