...
#define   Solenoide2_on    output_bit(PIN_D1,1)
#define   Solenoide2_off   output_bit(PIN_D1,0)
...
short EstadoDesfogue=OFF,EstadoAgua=OFF,EstadoSSR=OFF,PruebaEstados=OFF,Ciclo=OFF,flagCalentando=OFF,flagEsterilizando=OFF,flagAgua=OFF;
...
short flagBuzzer=OFF,flagSilencio=OFF,Finalizo=OFF,AlarmaPresion=OFF,ladoEsteril=OFF,ladoContaminado=OFF,flagc=OFF,guarda=OFF,EstadoBloqueo2=OFF;
short guardaEsteril=OFF,flagTrampa=OFF;
int8 openEsteril=10,openContaminado=10,esterilizo=10,AbreEsteril=0;

//Tiempos//---------------------
int16 TiempoCiclo=0,TiempoLlenado=0,TiempoSensadoNivel=0,Tp=0,Tc=0,tValveAgua=0,tmensaje=OFF;
...
float R1=100,R7=3000,R8=3100,R9=3100,R10=3100,AN=0,BN=0;
float X[2]={0.0,0.0};
...
int8 Control=0,tiemporecta=0,Controlando=0,ActivaImpresion=1,timpresion=2,CalculoImpresion=0,Inicio=1,Dato_Exitoso=0,Dato2=0,z=0,Alarma=0;
...
void temp1s(void){
   ...
   if(tiempos>=200)  // 12000 para que incremente cada minuto, 200para que incremente cada segundo.
   {
      ...
      if(IniciaCiclo && !Esterilizando && !Secando && !Desfogando && !Finalizo){
         TiempoCiclo++;
      }
...
void LogicaPuertas(){
   if(esterilizo==10){
      ladoContaminado=ON;
      ladoEsteril=OFF;
   }else if(esterilizo==5){
      ladoEsteril=ON;
      ladoContaminado=OFF;
   }
   
   if(DOOR){//Si puerta del lado contaminado esta cerrada
      if(AbreEsteril==5 && PresionCamara<2 && ladoEsteril){// Y realizo un ciclo de esterilizacion, Y la presion es menor a 2kPa, y oprimio desde pantalla
         Solenoide_on;
         Buzzer_on;
         delay_ms(5000);
         Solenoide_off;
...
//_--------------------------------------------------------------------------------------------------------------------------------------------------------
   if(Menu==1){ //Menu Principal.
      ...
      if(!DOOR && !guardaEsteril){
         esterilizo=10;
         write_eeprom(48,esterilizo);
         delay_ms(20);
         guardaEsteril=ON;
      }
      
      if(RX_Buffer[4]==0x22){
         if(PRES){//Si puerta del lado esteril esta cerrada
            if(!flagc){
               Solenoide_on;
               Buzzer_on;
               delay_ms(5000);
      ...
      if(RX_Buffer[3]==0x0f || UP){//0f, recibe caracteres ingresados desde el Display
         ...
         if(PantallaPrincipal==0){//Si esta en menu principal
            ...
            if(Opcion==8){// Cambio de Clave
               ...
               Op=OFF;
               Op2=OFF;
            }else if(Opcion==4){//Textil 
               printf("page Funcionamiento");
               SendDataDisplay();       
               ConfiguraCiclo(0,0,0,1,0,0,5,1,0,4,134);
               Op=OFF;
               Op2=OFF;
               MenuAnt=Menu;
               if(!flagTrampa){
...
//_--------------------------------------------------------------------------------------------------------------------------------------------------------      
      if(Menu==6){ // Menu de Prueba de Componentes
         ...
         if(RX_Buffer[4]==0x06)//Oprimio Solenoide2
         {
            EstadoBloqueo2=!EstadoBloqueo2;
            RX_Buffer[4]=0x00;
            RX_Buffer2[4]=0x00;
         }
...
//_--------------------------------------------------------------------------------------------------------------------------------------------------------    
   ...
   if(AlarmaPuerta || AlarmaTermostato || AlarmaLlenado || AlarmaSobreTemperatura || AlarmaTiempoProlongado || AlarmaEmergencia || AlarmaPresion){
      ActivaAlarma(2);
      if(RX_Buffer[4]==0x22){
         if(PRES && PresionCamara<2){//Si puerta del lado esteril esta cerrada
            if(!flagc){
               Solenoide_on;
               Buzzer_on;
               delay_ms(5000);
   ...
   {  
      ...
      if(flagImprime && ActivaImpresion==1){
         ...
         if(!CalderinLleno){
            ...
            /*if(tValveAgua<=400)
               AguaIn_on;
            else if(tValveAgua>400 && tValveAgua<600){   
               AguaIn_off;
               tValveAgua=0;
            }else if(tValveAgua>=600){
               AguaIn_off;
               tValveAgua=0;
            }*/
         }
                  ...
                  if(desfoguelento){
                     ...
                     if(PresionCamara<25){
                        ...
                        if(PresionCamara<2){
                           ...
                           esterilizo=5;
                           ActivaAlarma(2);
                           printf("Funcionamiento.t4f.txt=\" No abra puerta\"");
                           SendDataDisplay();
