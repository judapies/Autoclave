...
float LeerPT100(int media)
{
   ...
   V0 = V0/106.23; // Se elimina la ganancia
   //V0 += 14.82;
   V0 += 14.82;//5.039V 73.0mV
   Pt=R10/((1023/V0)-1);
...
void ControlaPresionPre(){
   if(Setpoint<120){
      if(PresionPrecamara>=Atmosferica+110){
         Suministro_off;
         Precalentamiento=ON;
      }else if(PresionPrecamara<Atmosferica+90){
         Suministro_on;
      }
   }else{
      if(PresionPrecamara>=Atmosferica+180){
         Suministro_off;
         Precalentamiento=ON;
      }else if(PresionPrecamara<Atmosferica+160){
         Suministro_on;
      }
   }
}
...
//Lectura EEPROM//--------------------------
   ...
   for(i=0;i<4;i++){//Lee valores almacenados de ciclos personalizados
      ...
      if(CiclosPersonalizados[i].Temperatura>128 || CiclosPersonalizados[i].Temperatura<105)
...
void LimitaValores(){
   ...
   if(desvio>2.0){
      desvio=0.5;write_eeprom(32,5);delay_ms(10);
   }
   if(Histeresis>3.0){
      Histeresis=0.1;write_eeprom(33,1);delay_ms(10);
   }
   if(Ciclos>60000){
      Ciclos=0;
      write_eeprom(37,0);
...
void codificaAlarma(){
   if(ACiclos[i]==0){
      ...
      printf("A%u.txt=\"PreCalentando\"",i+1);
      SendDataDisplay();
