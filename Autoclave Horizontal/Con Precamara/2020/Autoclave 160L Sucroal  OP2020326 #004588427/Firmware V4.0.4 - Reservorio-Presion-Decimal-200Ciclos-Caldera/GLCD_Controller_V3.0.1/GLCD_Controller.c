...
#define   Suministro_on    output_bit(PIN_D5,1)
#define   Suministro_off   output_bit(PIN_D5,0)
...
short flagDesfogando=OFF,Op=OFF,Op2=OFF,Minutos=OFF,Segundos=OFF,GuardaEEPROM=OFF,EstadoPaso=OFF,EstadoBuzzer=OFF,flagAlarma=OFF,Precalentamiento=OFF;
short flagBuzzer=OFF,flagSilencio=OFF,Finalizo=OFF,AlarmaPresion=OFF,Libre=OFF,purga=OFF,entro=OFF,Aire=OFF,disp_Guarda=OFF,guardaCiclo=OFF;
...
int8 h=0,l=0,Ajuste=0,Ajusten=0,MediaMovil=20,TTrampa=0,PantallaPrincipal=0,TipoClave=0,Modulo=0,Digito=0,yy=0,cicloVisto=0,cicloVisualizado=0,posicionDato=0,j=0,codigoAlarma=0,codigoAnt=0,Tapertura=0;
...
float Temperatura=0.0,K=0.0012858,Histeresis=0.0;
...
int8 Menu=0, MenuAnt=0,MenuAntt=240,i=0,CiclosL=0,CiclosH=0,CicloSeleccionado=0,desviacion=0,histe=0;
...
int16 t_exhaust=0,tbuzzer=0,tsilencio=0,tiempoAgua=0,tPurgaAire=0;
      ...
      if(Menu==6){ // Menu de Prueba de Componentes
         ...
         if(RX_Buffer[4]==0x21 || LEFT){//Oprimio salir
            ...
            if(RX_Buffer[4]==0x21 || LEFT){
               ...
               SendDataDisplay();
               PantallaPrincipal=0;
               PruebaEstados=OFF;Desfogue_off;Paso_off;Suministro_off;SSR_off;
               ApagaSalidas();
               // Falta apagar Todo
            }
         }
...
//_--------------------------------------------------------------------------------------------------------------------------------------------------------    
   ...
   if(PresionCamara>(int16)(Atmosferica)+242 || PresionPreCamara>(int16)(Atmosferica)+260){
      AlarmaPresion=ON;
   }
   ...
   if(Grafica==1){
      printf("add 14,0,%1.0f",Temperatura/1.667);
      SendDataDisplay();
      printf("add 14,1,%1.0f",PresionCamara/1.667);
      SendDataDisplay();
      printf("add 14,2,%1.0f",PresionPreCamara/1.667);
      SendDataDisplay();
   ...
   {                            
      ...
      printf("fecha.txt=\"T:%2.1f Fecha:%02u/%02u/20%02u    Hora: %02u:%02u\"",Tactual,Dia,Mes,Year,Hora,Minuto);
      SendDataDisplay();
   }
      ...
      if(RX_Buffer[4]==0x11 || (RIGHT && LEFT)){//Tecla Regresar
         ...
         if(RX_Buffer[4]==0x11 || (RIGHT && LEFT)){
            if(!GuardaEEPROM){
               ...
               write_eeprom(32,histe);
               delay_ms(10);
               desviacion=(int8)(desvio*10);
               write_eeprom(33,desviacion);
               delay_ms(10);
               write_eeprom(40,Tapertura);
               delay_ms(10);
               GuardaEEPROM=ON;
            }
            printf("page MenuPrincipal");
