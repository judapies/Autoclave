...
void Controla(){
   ...
   */
   ...
   if(Tactual>Setpoint+0.3){
      suministroVapor=OFF;
      Paso=ON;
   }else if(Tactual<=Setpoint+0.2){
      suministroVapor=ON;
      Paso=ON;
   }
   ////////////////////////////////////
...
void ControlaPresion(){
   if(Setpoint>125){
      if(PresionPrecamara>=(300-Atmosferica)){
         suministroVapor=OFF;
         Precalentamiento=ON;
      }
      if(PresionPrecamara<(290-Atmosferica))
         suministroVapor=ON;
   }else{
      if(PresionPrecamara>=(230-Atmosferica)){
         suministroVapor=OFF;
         Precalentamiento=ON;
      }
      if(PresionPrecamara<(220-Atmosferica))
         suministroVapor=ON;
   }
}
...
void cierraPuertaS(){// Añadir logica de puertas
   abrirPuertaS=OFF;
   tVacioEmpaqueS=0;
   if(!BLOQUEOS){
      cilindroSucio=OFF;
...
void cierraPuertaL(){// Añadir logica de puertas
   abrirPuertaL=OFF;
   tVacioEmpaqueL=0;
   if(!BLOQUEOL){
      cilindroLimpio=OFF;
