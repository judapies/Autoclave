...
float V0=0.0,R1=6200.0,Pt,Tciclo=10.0,Tpulso=0.0,error=0.0,Prom=0.0,Temperature=0.0,tmp=0.0,Ganancia=25.0,Ganancia2=20.0,desvio=1.0,d3=0.0,d4=0.0,d5=0.0;
...
float Leer_Sensor(int media)
{
   ...
   V0 = V0/40.11; // Se elimina la ganancia
...
void main()
{
   ...
   sensores(0);
   /*desvio=read_eeprom(11);Tciclo=read_eeprom(12);*/
...
//_--------------------------------------------------------------------------------------------------------------------------------------------------------    
   ...
   {      
      ...
      if(Control==1 && !DOOR && !STOP && AlarmaAgua==0 && Alarma==0) // Si esta en condiciones normales para ejecutar el control.
      {
         ...
         if(CamaraOUT==1 || flag_presostato==0) // Si desea abrir electrovalvula desde Software de control 
         {
            EV_Desfogue_on;
         }
         else
