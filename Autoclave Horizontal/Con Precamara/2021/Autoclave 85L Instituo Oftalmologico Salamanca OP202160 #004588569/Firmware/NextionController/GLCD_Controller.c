...
#define   EMPAQUES      !input(PIN_B1)
#define   EMPAQUEL      !input(PIN_B2)
#define   DOORS         !input(PIN_B3)
#define   DOORL         !input(PIN_B4)
#define   LOWB          !input(PIN_B5)
...
short AlarmaBomba=OFF,guardaCiclo=OFF,Cilindro=OFF,AlarmaLlenado=OFF,cerroSucio=OFF,cerroLimpio=OFF,pulsoSubirS=OFF,pulsoSubirL=OFF;
int16 TiempoVacio=600;

//Banderas//--------------------
short EstadoAguaEnfriador=OFF,EstadoSuministroV=OFF,EstadoCilindrosL=OFF,EstadoCilindrosS=OFF,EstadoEmpaqueL=OFF,EstadoEmpaqueS=OFF;
...
//Tiempos//---------------------
int16 TiempoCiclo=0,TiempoSensadoNivel=0,Tp=0,Tc=0,tguarda=0,tNivelBajo=0,TiempoLlenado=OFF;
...
int16 CantidadPulsos=4,tEmpaqueS=0,tEmpaqueL=0;
int8 Setpoint=0,Opcion=1,Opcion2=1,tiempo_esterilizacion=0,tiempo_secado=0;
int16 tiempos=0,tiempos2=0,tiempos3=0,tiempoctrl=0,Nivel=0,tinicio=0,PresionCamara=0,PresionPreCamara=0,Ciclos=0,tbomba=0,tvacio=0,tiempoImpresion=0;
   ...
   if(Menu==1){ //Menu Principal.
      ...
      if(RX_Buffer[4]==0x01 && RX_Buffer[5]==0x0f && estados[19+RX_Buffer[4]]){//01, Calentamiento
         ...
         if(RX_Buffer[4]==0x01 && RX_Buffer[5]==0x0f && estados[19+RX_Buffer[4]]){//01, Calentamiento
            ...
            SendDataDisplay();
            ConfiguraCiclo(0,15,0,1,0,1,121,4,25,0);
            Op=OFF;
            Op2=OFF;
            MenuAnt=Menu;
      ...
      if(RX_Buffer[4]==0x02 && RX_Buffer[5]==0x0f && estados[19+RX_Buffer[4]]){//02, No Envueltos 134
         ...
         if(RX_Buffer[4]==0x02 && RX_Buffer[5]==0x0f && estados[19+RX_Buffer[4]]){
            printf("page Funcionamiento");
            SendDataDisplay();       
            ConfiguraCiclo(0,3,0,1,0,2,134,4,25,0);            
            Op=OFF;
            Op2=OFF;
            MenuAnt=Menu;
            //TiempoVacio=CantidadPulsos*180;
      ...
      if(RX_Buffer[4]==0x03  && RX_Buffer[5]==0x0f && estados[19+RX_Buffer[4]]){//03, No Envueltos 121
         ...
         if(RX_Buffer[4]==0x03 && RX_Buffer[5]==0x0f && estados[19+RX_Buffer[4]]){
            printf("page Funcionamiento");
            SendDataDisplay();     
            ConfiguraCiclo(0,15,0,1,0,3,121,4,25,0);  
            Op=OFF;
            Op2=OFF;
            MenuAnt=Menu;
            //TiempoVacio=CantidadPulsos*180;
      ...
      if(RX_Buffer[4]==0x04 && RX_Buffer[5]==0x0f && estados[19+RX_Buffer[4]]){//04, Envueltos 134
         ...
         if(RX_Buffer[4]==0x04 && RX_Buffer[5]==0x0f && estados[19+RX_Buffer[4]]){
            printf("page Funcionamiento");
            SendDataDisplay();       
            ConfiguraCiclo(0,4,0,15,0,4,134,4,25,0);  
            Op=OFF;
            Op2=OFF;
            MenuAnt=Menu;
            //TiempoVacio=CantidadPulsos*180;
      ...
      if(RX_Buffer[4]==0x05 && RX_Buffer[5]==0x0f && estados[19+RX_Buffer[4]]){//05, Envueltos 121
         ...
         if(RX_Buffer[4]==0x05 && RX_Buffer[5]==0x0f && estados[19+RX_Buffer[4]]){
            printf("page Funcionamiento");
            SendDataDisplay();       
            ConfiguraCiclo(0,15,0,15,0,5,121,4,25,0);  
            Op=OFF;
            Op2=OFF;
            MenuAnt=Menu;
            CantidadPulsos=7;
      ...
      if(RX_Buffer[4]==0x06 && RX_Buffer[5]==0x0f && estados[19+RX_Buffer[4]]){//06, Envuelto Doble 1
         ...
         if(RX_Buffer[4]==0x06 && RX_Buffer[5]==0x0f && estados[19+RX_Buffer[4]]){
            printf("page Funcionamiento");
            SendDataDisplay();       
            ConfiguraCiclo(0,7,0,20,0,6,134,4,25,0);  
            Op=OFF;
            Op2=OFF;
            MenuAnt=Menu;
            //TiempoVacio=CantidadPulsos*180;
      ...
      if(RX_Buffer[4]==0x07 && RX_Buffer[5]==0x0f && estados[19+RX_Buffer[4]]){//07, Envuelto Doble 2
         ...
         if(RX_Buffer[4]==0x07 && RX_Buffer[5]==0x0f && estados[19+RX_Buffer[4]]){
            printf("page Funcionamiento");
            SendDataDisplay();       
            ConfiguraCiclo(0,7,0,30,0,7,134,4,25,0);  
            Op=OFF;
            Op2=OFF;
            MenuAnt=Menu;
            //TiempoVacio=CantidadPulsos*180;
      ...
      if(RX_Buffer[4]==0x08 && RX_Buffer[5]==0x0f && estados[19+RX_Buffer[4]]){//08, Prion
         ...
         if(RX_Buffer[4]==0x08 && RX_Buffer[5]==0x0f && estados[19+RX_Buffer[4]]){
            printf("page Funcionamiento");
            SendDataDisplay();       
            ConfiguraCiclo(0,20,0,15,0,8,134,4,25,0);  
            Op=OFF;
            Op2=OFF;
            MenuAnt=Menu;
            //TiempoVacio=CantidadPulsos*180;
      ...
      if(RX_Buffer[4]==0x09 && RX_Buffer[5]==0x0f && estados[19+RX_Buffer[4]]){//09, Bowie & Dick
         ...
         if(RX_Buffer[4]==0x09 && RX_Buffer[5]==0x0f && estados[19+RX_Buffer[4]]){
            printf("page Funcionamiento");
            SendDataDisplay();       
            ConfiguraCiclo(30,3,0,1,0,9,134,4,25,0);  
            Op=OFF;
            Op2=OFF;
            MenuAnt=Menu;
            //TiempoVacio=CantidadPulsos*180;
      ...
      if(RX_Buffer[4]==0x0a && RX_Buffer[5]==0x0f && estados[19+RX_Buffer[4]]){//0a, Test de Fugas
         ...
         if(RX_Buffer[4]==0x0a && RX_Buffer[5]==0x0f && estados[19+RX_Buffer[4]]){
            printf("page Funcionamiento");
            SendDataDisplay();       
            ConfiguraCiclo(0,20,0,0,0,6,30,1,17,1);
            MenuAnt=Menu;
            //TiempoVacio=CantidadPulsos*180;
         }
      }
...
//_--------------------------------------------------------------------------------------------------------------------------------------------------------      
    if(Menu==6){ // Menu de Prueba de Componentes
         ...
         if(RX_Buffer[4]==0x0d){//Oprimio Empaque
...
//_--------------------------------------------------------------------------------------------------------------------------------------------------------    
   ...
   {                      
      ...
      printf("fecha.txt=\"%s Fecha:%02u/%02u/20%02u   Hora: %02u:%02u\"",usuario,Dia,Mes,Year,Hora,Minuto);
      SendDataDisplay();
   }
