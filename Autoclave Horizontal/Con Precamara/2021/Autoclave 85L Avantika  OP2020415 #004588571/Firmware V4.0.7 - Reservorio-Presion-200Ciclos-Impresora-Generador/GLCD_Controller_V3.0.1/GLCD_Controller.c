...
//-----Defines para configuracion de logica de funcionamiento de autoclave---//
#define PT100Fija //Define para autcolaves con reservorio y Pt100 fija (NO FLEXIBLE)
//#define Reservorio //Define para autoclaves con reservorio
#define Tmax 134
#define Tmin 105
//#define ULKA
#define Generador
...
#define   RIGHT         !input(PIN_B1)
#define   LEFT          !input(PIN_B0)
...
#else
#define   Bomba_on         output_bit(PIN_C0,0)
#define   Bomba_off        output_bit(PIN_C0,1)
#endif
...
//Banderas//--------------------
short IniciaLlenado=OFF,CalderinLleno=OFF,IniciaCiclo=OFF,Esterilizando=OFF,Secando=OFF,SensadoNivel=OFF,Desfogando=OFF,DesfogueSuave=OFF,Precalentamiento=OFF;
short EstadoDesfogue=OFF,EstadoAgua=OFF,EstadoSSR=OFF,PruebaEstados=OFF,Ciclo=OFF,flagCalentando=OFF,flagEsterilizando=OFF,EstadoAire=OFF,SensadoNivelL=OFF;
short flagDesfogando=OFF,Op=OFF,Op2=OFF,Minutos=OFF,Segundos=OFF,GuardaEEPROM=OFF,EstadoPaso=OFF,EstadoBuzzer=OFF,flagAlarma=OFF,NivelBajo=OFF,purgo=OFF,purgaAire2=OFF;
short flagBuzzer=OFF,flagSilencio=OFF,Finalizo=OFF,AlarmaPresion=OFF,Libre=OFF,purga=OFF,entro=OFF,Aire=OFF,disp_Guarda=OFF,guardaCiclo=OFF,flagBombaAgua=OFF,flagBombaAguaOff=OFF;

//Tiempos//---------------------
int16 TiempoCiclo=0,TiempoLlenado=0,TiempoSensadoNivel=0,Tp=0,Tc=0,tguarda=0,tNivelBajo=0,conteoPresion=0,tPurgaAire=0;
...
float X[2]={0.0,0.0};
float Xx[2]={0.0,0.0};
float promediopresion=0.0,Aumento=0.0,Aumento2=0.0,promediopresionp=0.0;
int8 h=0,l=0,Ajuste=0,Ajusten=0,MediaMovil=20,TTrampa=0,PantallaPrincipal=0,TipoClave=0,Modulo=0,Digito=0,yy=0,cicloVisto=0,cicloVisualizado=0,posicionDato=0,j=0,codigoAlarma=0,codigoAnt=0;
float V0=0.0,Pt,Tciclo=10.0,Tpulso=0.0,error=0.0,Prom=0.0,Temperature=0.0,tmp=0.0,Ganancia=25.0,Ganancia2=20.0,desvio=1.2,tmp2=0.0;
...
//Varios//--------------------------------
short desfoguelento=0,Lectura=0,Grafica=0,LOW=0,HIG=0;
...
int8 Setpoint=0,Opcion=1,Opcion2=1,tiempo_esterilizacion=0,tiempo_secado=0,tiempoImpresion=0,temperaturaInicial=0;
int16 tiempos=0,tiempos2=0,tiempos3=0,tiempoctrl=0,Nivel=0,tinicio=0,PresionCamara=0,PresionPreCamara=0,Ciclos=0,NivelSeleccionado=0,tBombaAgua=0,tBombaAguaOff=0;
signed int  Contrasena[4]={0,0,0,0};      // Contrasena de 4 digitos
signed int  Password[4]={0,0,0,0};        // Contrasena Almacenada de 4 digitos
int8 r=0,t=0,r22=0,t2=0,TiempoControl=0;
int8 y=0,ConteoBuzzer=0;
int16 t_exhaust=0,tbuzzer=0,tsilencio=0,tiempoAgua=0,tStop=0,tTermostato=0;
...
int8 iconoGrande[9]={25,16,22,24,23,28,20,46,69};
int8 iconoPeque[9]={34,36,18,33,32,38,19,17,68};
//int8 iconoGrande[9]={86,88,80,84,82,90,78,76,92};
//int8 iconoPeque[9]={85,87,79,83,81,89,77,75,91};
char const texto[9][20]={"LIQUIDOS A","CAUCHO","INSTRUMENTAL","LIQUIDOS B","LIBRE","TEST","HORA Y FECHA","CLAVE","DATOS"};
   ...
   while(true)
   {
      ...
      // Monitoreo Nivel de Agua//--------------------------------------
            if(!LOW){//Si no sensa agua en el nivel alto y no se ha llenado el calderin         
               ...
               #ifdef ULKA
                  if(!PruebaEstados){
                     if(PresionCamara<=((int16)Atmosferica+10) && Menu!=20 && HIG && Menu!=255){
                        enciendeBombaAgua();
                     }
                     if(!HIG){
                        apagaBombaAgua();
...
//_--------------------------------------------------------------------------------------------------------------------------------------------------------
   if(Menu==1){ //Menu Principal.
      //printf("fecha.txt=\"T:%3.1f°C Fecha:%02u/%02u/20%02u Hora: %02u:%02u:%02u\"",Temperatura,Dia,Mes,Year,Hora,Minuto,Segundo);
      //SendDataDisplay();
      revisePresion();
      PruebaEstados=OFF;
      GuardaEEPROM=OFF;
      if(RX_Buffer[3]==0x0f || UP){//0f, recibe caracteres ingresados desde el Display
         ...
         if(PantallaPrincipal==0){//Si esta en menu principal
            if(RX_Buffer[4]==0x37){//Ajustes y pasa a solicitud de password de tecnico
               ...
               RX_Buffer2[3]=0x00;
               Opcion=1;
            }else if(Opcion==8){// Cambio de Clave
               printf("page Clave");
               SendDataDisplay(); 
               Op=OFF;
               Op2=OFF;
               ...
               Op2=OFF;
               MenuAnt=2;
            }else if(Opcion==1){//LiquidosA
               printf("page Nivel");
               SendDataDisplay();       
               ConfiguraCiclo(0,0,5,1,0,0,0,0,1,1,121);
               Op=OFF;
               Op2=OFF;
               MenuAnt=2;
            }else if(Opcion==3){//Instrumental
               printf("page Funcionamiento");
               SendDataDisplay();       
               ConfiguraCiclo(0,0,0,3,0,0,0,3,0,3,121);
               Op=OFF;
               Op2=OFF;
               MenuAnt=Menu;
               //if(Temperatura>60)TTrampa=TTrampa+5;
            }else if(Opcion==2){//Caucho
               printf("page Funcionamiento");
               SendDataDisplay();       
               ConfiguraCiclo(0,0,5,1,0,0,3,0,0,2,121);
               Op=OFF;
               Op2=OFF;
               MenuAnt=Menu;
               //if(Temperatura>60)TTrampa=TTrampa+5;
      ...
      if(Menu==6){ // Menu de Prueba de Componentes
         ...
         if(RX_Buffer[4]==0x21 || LEFT){//Oprimio salir
            ...
            if(RX_Buffer[4]==0x21 || LEFT){
               ...
               SendDataDisplay();
               PantallaPrincipal=0;
               PruebaEstados=OFF;Desfogue_off;Agua_off;
               #ifdef Generador
               Paso_off;
               #else
               Bomba_off;
...
//_--------------------------------------------------------------------------------------------------------------------------------------------------------      
   if(Menu==12){//Menu de Visualizacion de Ciclos Realizados
      ...
      */      
      ...
      if(RX_Buffer[6]==0x02 || LEFT){//02, Izquierda
         RX_Buffer[6]=0x00;  
         RX_Buffer2[6]=0x00;
         if(cicloVisto>0)
            cicloVisto--;
      ...
      if(RX_Buffer[6]==0x03 || UP){//03, Arriba
         RX_Buffer[6]=0x00;  
         RX_Buffer2[6]=0x00;
      ...
      if(RX_Buffer[6]==0x04 || DOWN){//04, Abajo
         RX_Buffer[6]=0x00;  
         RX_Buffer2[6]=0x00;
      ...
      if(RX_Buffer[6]==0x05 || RIGHT){//05, Derecha
         RX_Buffer[6]=0x00;  
         RX_Buffer2[6]=0x00;
         if(cicloVisto<200)
            cicloVisto++;
...
//_--------------------------------------------------------------------------------------------------------------------------------------------------------    
   ...
   #ifdef Generador
   if(PresionPreCamara>=275 || PresionCamara>=275){
      conteoPresion++;
      if(PresionPreCamara>=275 || PresionCamara>=275){
         if(conteoPresion>=50)
            AlarmaPresion=ON;  
   ...
   #else
   if(PresionCamara>Atmosferica+275){
      AlarmaPresion=ON;
   }
   #endif
   
   if(Temperatura>105 && PresionCamara<Atmosferica+10 && !Desfogando){
      AlarmaCiclo=ON;
   }
   ...
   if(TiempoCiclo>3000){
      ...
      if(PresionPreCamara<Atmosferica+10){
         AlarmaTiempoProlongado=ON;
      }
   }else{
   ...
   || AlarmaCiclo){
      ...
      if(flagImprime){
         ...
         if(!CalderinLleno){
               ...
               SendDataDisplay();
               printf("Funcionamiento.t5f.txt=\" Llenando Generador\"");
               SendDataDisplay();
            #else
            #ifdef Reservorio
               if(!HIG){
                  printf("Funcionamiento.t4f.txt=\" Reservorio Bajo \"");
                  SendDataDisplay();
               ...
               (Testerilizacion[2]==0)&&(Testerilizacion[3]==0)){ 
                  if(desfoguelento){
                     ...
                     if(PresionCamara<=Atmosferica+25){
                        ...
                        }else if(PresionCamara>=Atmosferica+7){
                           Desfogue_on;
                           #ifdef Generador
                           //Paso_on;
                           Paso_off;
                        ...
                        #ifdef Generador
                        //Paso_on;
                        Paso_off;//Modificado por que esta autoclave no tiene precamara
                        #else
                        Bomba_off;
                        #endif
                        Aire=OFF;
                     ...
                     Controlando=0;
                     Control=0;
                     if(PresionCamara<=Atmosferica+1){
                     ...
                     }else if(PresionCamara>Atmosferica+6){
                        codigoAlarma=13;
                        if(codigoAnt!=codigoAlarma){
                           codigoAnt=codigoAlarma;
                           GuardaDatoCiclo();                  
   ...
   if(Grafica==1){
      ...
      SendDataDisplay();
      //printf("add 14,1,%1.0f",PresionCamara/1.667);
      //SendDataDisplay();
      //printf("add 14,0,%1.0f",Temperatura);
      //SendDataDisplay();
      Grafica=0;
...
//_--------------------------------------------------------------------------------------------------------------------------------------------------------
      ...
      if(Opcion2==1){
         ...
         SendDataDisplay();   
         printf("t3c.txt=\"%02u\"",Atmosferica);
         SendDataDisplay();
