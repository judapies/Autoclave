...
//-------------------------------Control de Calentamiento------------------------------------------------------------//         
   if(Esterilizando){
      ...
      error=desvio+1.0+Setpoint-Tactual;      // Cálculo del error ******
   }
   
   if(error>1.0 && error<20 && Aumento2<0.3){
      if(Tpulso<=6.0){
         flagTC=1;
         if(TiempoControl>=Derivativo){
            TiempoControl=0;
...
void LimitaValores(){
   ...
   if(Derivativo>20.0 || Derivativo<5){
      Derivativo=15.0;
      write_eeprom(27,15);
      delay_ms(10);
   }
...
void GuardaDatoEstatico(){
   ...
   if(!disp_Guarda){
      ...
      if(!Libre)
         ...
         write_ext_eeprom(1+(cicloMemoria*320),CicloLibre+4);
      delay_ms(10);
...
#ifdef Generador
void ControlaPresionChaqueta(){
   if(Setpoint<125){
      if(PresionPreCamara<110){
         SSR_on;
         SSR2_on;
      }else if(PresionPreCamara>120){
         SSR_off;
         SSR2_off;
      ...
      if(PresionPreCamara<200){
         SSR_on;
         SSR2_on;
      }else if(PresionPreCamara>215){
         SSR_off;
         SSR2_off;
         if(!Precalentamiento && guardaCiclo){
            Precalentamiento=ON;
...
void enciendeBombaAgua(){
   if(tBombaAgua>120){//Enciende 120 segundos
      Agua_off;
      Bomba_off;
      flagBombaAgua=OFF;
      flagBombaAguaOff=ON;
      if(tBombaAguaOff>=60){//Apaga 60 segundos
         tBombaAgua=0;
      }
   }else{
      Agua_on;
...
void revisePresion(){
   if(DOOR){
         if(PresionCamara>=Atmosferica+22){
            ActivaAlarma(3);
            if(flagBuzzer){
               printf("fecha.txt=\"Pc:%03Ld kPa Pp:%03Lu kPa T:%2.1f°C Hora:%02u:%02u:%02u\"",PresionCamara,PresionPrecamara,Temperatura,Hora,Minuto,Segundo);
               SendDataDisplay();
            }else{
...
void purgaAire(){
   ...
   if(purgaAire2){
      if(tPurgaAire>0 && tPurgaAire<6){
         Desfogue_on;
      }else if(tPurgaAire>=6 && tPurgaAire<66){
         Desfogue_off;
      }else if(tPurgaAire>=66 && tPurgaAire<72){
         Desfogue_on;
      }else if(tPurgaAire>=72 && tPurgaAire<132){
         Desfogue_off;
      }else if(tPurgaAire>=132 && tPurgaAire<138){
         Desfogue_on;
      }else if(tPurgaAire>=138 && tPurgaAire<198){
         Desfogue_off;
      }else if(tPurgaAire>=198 && tPurgaAire<204){
         Desfogue_on;
