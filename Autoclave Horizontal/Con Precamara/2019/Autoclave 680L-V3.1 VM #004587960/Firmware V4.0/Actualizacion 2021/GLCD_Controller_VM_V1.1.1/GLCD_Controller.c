...
#define   Agua2_on         output_bit(PIN_D7,1)
#define   Agua2_off        output_bit(PIN_D7,0)
...
short AlarmaBomba=OFF,guardaCiclo=OFF,Cilindro=OFF;
...
//Varios//--------------------------------
short desfoguelento=0,Lectura=0,Grafica=0,borra=OFF;
...
int16 CantidadPulsos=4,tStop=0,conteoPresion=0,i=0;
...
int16 tbuzzer=0,tsilencio=0,posicionDato=0,cicloVisualizado=0,cicloVisto=0;
...
void main()
{
   ...
   while(true)
   {
      ...
      if(!PruebaEstados){
         if(Temperatura2>40)
            Agua2_on;
         else if(Temperatura2<38)
            Agua2_off;
      }
      
      if(!PruebaEstados){
         if(PresionCamara<=Atmosferica+3 && (!IniciaCiclo || Finalizo)){
            sistemaBloqueo(0);
         }else{
            sistemaBloqueo(1);
         }
...
// Monitoreo Nivel de Agua//--------------------------------------
            if(!LOW){//Si no sensa agua en el reservorio del agua para la bomba
...
//_--------------------------------------------------------------------------------------------------------------------------------------------------------
   if(Menu==8){ //Menu Ciclos Personalizados
      ...
      if(RX_Buffer[4]==0x70 && CicloLibre>0){//70, Iniciar Ciclo
         ...
         ConfiguraCiclo(CiclosPersonalizados[CicloLibre-1].Segundoest-((CiclosPersonalizados[CicloLibre-1].Segundoest/10)*10),
                        CiclosPersonalizados[CicloLibre-1].Segundoest/10,
                        CiclosPersonalizados[CicloLibre-1].Minutoest-((CiclosPersonalizados[CicloLibre-1].Minutoest/10)*10),
                        CiclosPersonalizados[CicloLibre-1].Minutoest/10,
                        CiclosPersonalizados[CicloLibre-1].Segundosec-((CiclosPersonalizados[CicloLibre-1].Segundosec/10)*10),
                        CiclosPersonalizados[CicloLibre-1].Segundosec/10,
                        CiclosPersonalizados[CicloLibre-1].Minutosec-((CiclosPersonalizados[CicloLibre-1].Minutosec/10)*10),
...
//_--------------------------------------------------------------------------------------------------------------------------------------------------------      
   if(Menu==12){//Menu de Visualizacion de Ciclos Realizados
      ...
      printf("t0.txt=\"Ciclo %03Lu\"",cicloVisto+1);
      SendDataDisplay();
...
//_--------------------------------------------------------------------------------------------------------------------------------------------------------      
    if(Menu==14){//Menu de Activacion de Ciclos
      ...
      for(i=0;i<30;i++){
         ...
         if(estados[i]){
            printf("b%Lu.bco=2016",i);
            SendDataDisplay();             
         }else{
            printf("b%Lu.bco=63488",i);
            SendDataDisplay(); 
         }
      }
   }
...
//_--------------------------------------------------------------------------------------------------------------------------------------------------------    
   ...
   if(TiempoCiclo>3600 && !Fugas){//ANALIZAR BIEN ESTA ALARMA
   ...
   if(AlarmaPuerta || AlarmaTermostato || AlarmaSobreTemperatura || AlarmaTiempoProlongado || AlarmaEmergencia || AlarmaPresion || AlarmaBomba 
         ...
         if(!Vacio){ //Si no ha realizado los pulsos de vacio
            ...
            if(!flag_vac){//Ciclo inicial para purga de Bomba
               ...
               if(CalderinLleno)
                  ...
                  BombaVacio_off;
               IniciaVacio2=ON;
               PulsoPositivo=5; //Prueba para ver rendimiento de bomba de vacio************
            }
            printf("Funcionamiento.t4f.txt=\" Ninguna\"");
            SendDataDisplay();
            ControlaPresion();
            ...
            if(PulsoPositivo==5){//Pulso Positivo
               tvacio=0;
               flag_pulso=0;
               printf("Funcionamiento.t5f.txt=\" Pulso Positivo %u\"",PulsosVacio+1);
               SendDataDisplay();
               if(CalderinLleno)
                  BombaVacio_on;
               else
      ...
      IniciaCiclo=ON;
         ...
         if((Testerilizacion[0]==0)&&(Testerilizacion[1]==0)&&(Testerilizacion[2]==0)&&(Testerilizacion[3]==0)){                
            IniciaVacio=OFF;
            tbomba=0;
            BombaVacio_off;
            Vacio_off;
   ...
   {                      
      Lectura=0;
      printf("tuser.txt=\"Ciclo #%05Lu  %s\"",Ciclos,usuario);
      SendDataDisplay();
