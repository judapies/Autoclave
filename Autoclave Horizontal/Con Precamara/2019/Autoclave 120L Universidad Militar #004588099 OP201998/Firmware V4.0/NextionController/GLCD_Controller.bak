...
#define   HIG           !input(PIN_B1)
#define   LOWB          !input(PIN_B2)
#define   DOOR          !input(PIN_B3)
#define   STOP          !input(PIN_B4)
#define   TERM          !input(PIN_B5)
#define   PRES          input(PIN_B6)
#define   BLOQUEO       !input(PIN_B7)

#define   Suministro_on    output_bit(PIN_D1,0)
#define   Suministro_off   output_bit(PIN_D1,1)

#define   Desfogue_on      output_bit(PIN_D0,0)
#define   Desfogue_off     output_bit(PIN_D0,1)

#define   Paso_on          output_bit(PIN_C2,0)
#define   Paso_off         output_bit(PIN_C2,1)

#define   Vacio_on         output_bit(PIN_C1,0)
#define   Vacio_off        output_bit(PIN_C1,1)

#define   Aire_on          output_bit(PIN_C0,0)
#define   Aire_off         output_bit(PIN_C0,1)

#define   BombaVacio_on    output_bit(PIN_D2,0)
#define   BombaVacio_off   output_bit(PIN_D2,1)

#define   Agua_on          output_bit(PIN_D3,0)
#define   Agua_off         output_bit(PIN_D3,1)

#define   Agua2_on         output_bit(PIN_C5,0)
#define   Agua2_off        output_bit(PIN_C5,1)

#define   Cilindro1_on     output_bit(PIN_D7,1)
#define   Cilindro1_off    output_bit(PIN_D7,0)

#define   Cilindro2_on     output_bit(PIN_D6,1)
#define   Cilindro2_off    output_bit(PIN_D6,0)

#define   Empaque_on       output_bit(PIN_D5,1)
#define   Empaque_off      output_bit(PIN_D5,0)

#define   Aux_on           output_bit(PIN_D4,1)
#define   Aux_off          output_bit(PIN_D4,0)
...
short AlarmaBomba=OFF,guardaCiclo=OFF,Cilindro=OFF,AlarmaLlenado=OFF,pulsoSubir=OFF,pulsoBajar=OFF,AlarmaEmpaque=OFF;
int16 TiempoVacio=600;

//Banderas//--------------------
short IniciaLlenado=OFF,IniciaCiclo=OFF,Esterilizando=OFF,Secando=OFF,SensadoNivel=OFF,Desfogando=OFF,DesfogueSuave=OFF,CalderinLleno=OFF,abrirPuerta=OFF,cerro=OFF,vacioEmpaque=OFF;
short EstadoDesfogue=OFF,EstadoSuministro=OFF,EstadoAire=OFF,PruebaEstados=OFF,disp_Guarda=OFF,Ciclo=OFF,EstadoAgua2=OFF,EstadoAux=OFF;
...
//Tiempos//---------------------
int16 TiempoCiclo=0,TiempoSensadoNivel=0,Tp=0,Tc=0,tguarda=0,tNivelBajo=0,TiempoLlenado=0,tVacioEmpaque=0,tEmpaque=OFF;
...
float K=0.0012858;
float R10=6765,AN=0,BN=0;
float X[2]={0.0,0.0};
float promediopresion=0.0,promediopresionp=0.0;
int8 h=0,l=0,MediaMovil=20,PantallaPrincipal=0,TipoClave=0,Modulo=0,TiempoSensadoNivelR=0;
...
int16 CantidadPulsos=4,PresionFinal=0;
int8 Setpoint=0,Opcion=1,Opcion2=1,tiempo_esterilizacion=0,tiempo_secado=0;
int16 tiempos=0,tiempos2=0,tiempos3=0,tiempoctrl=0,Nivel=0,tinicio=0,PresionCamara=0,PresionPreCamara=0,Ciclos=0,tbomba=0,tvacio=0,tiempoImpresion=0,t_exhaust=0,NivelSeleccionado=0;
signed int  Password[4]={0,0,0,0};        // Contrasena Almacenada de 4 digitos
int8 r=0,t=0,r22=0,t2=0,NivelDes=0;
int8 y=0,ConteoBuzzer=0,posicionDato=0,codigoAlarma=0,codigoAnt=0,codigoUsuario2=0;
int16 tbuzzer=0,tsilencio=0,TiempoControl=0;
...
int8 YearTx=0,MesTx=0,DiaTx=0,HoraTx=0,MinutoTx=0,SegundoTx=0,dowTx=0,SetTime=0,Imprime=0,Pulsos=0,j=0,codigoUsuario=0,codigoUsuarioant=0;
...
void temp1s(void){
   ...
   if(tiempos>=200){  // 12000 para que incremente cada minuto, 200para que incremente cada segundo.
      ...
      if(IniciaCiclo && !Esterilizando && DOOR)
         TiempoCiclo++;
...
void main()
{
   ...
   SendDataDisplay();
   ApagaSalidas();
   //TiempoVacio=CantidadPulsos*360; 
   TiempoVacio=360; 
   Sensor[0].Gain=107.74;
   Sensor[1].Gain=107.74;
   Imprime=10;
...
//_--------------------------------------------------------------------------------------------------------------------------------------------------------
   if(Menu==1){ //Menu Principal.
      ...
      if(RX_Buffer[4]==0x03  && RX_Buffer[5]==0x0f && estados[19+RX_Buffer[4]]){//03, Contenedores 
         delay_ms(5);
         if(RX_Buffer[4]==0x03 && RX_Buffer[5]==0x0f && estados[19+RX_Buffer[4]]){
            printf("page Funcionamiento");
            SendDataDisplay();     
            ConfiguraCiclo(0,5,0,25,0,3,121,7,25,0);  
            Op=OFF;
            Op2=OFF;
            MenuAnt=Menu;
            //TiempoVacio=CantidadPulsos*180;
      ...
      if(RX_Buffer[4]==0x04 && RX_Buffer[5]==0x0f && estados[19+RX_Buffer[4]]){//04, Rapid
         delay_ms(5);
         if(RX_Buffer[4]==0x04 && RX_Buffer[5]==0x0f && estados[19+RX_Buffer[4]]){
            printf("page Funcionamiento");
            SendDataDisplay();       
            ConfiguraCiclo(0,5,0,8,0,4,134,7,25,0);  
            Op=OFF;
            Op2=OFF;
            MenuAnt=Menu;
            //TiempoVacio=CantidadPulsos*180;
      ...
      if(RX_Buffer[4]==0x08 && RX_Buffer[5]==0x0f && estados[19+RX_Buffer[4]]){//08, Prion
         ...
         if(RX_Buffer[4]==0x08 && RX_Buffer[5]==0x0f && estados[19+RX_Buffer[4]]){
            printf("page Funcionamiento");
            SendDataDisplay();       
            ConfiguraCiclo(0,20,0,15,0,8,134,7,25,0);  
            Op=OFF;
            Op2=OFF;
            MenuAnt=Menu;
            //TiempoVacio=CantidadPulsos*180;
      ...
      if(RX_Buffer[4]==0x09 && RX_Buffer[5]==0x0f && estados[19+RX_Buffer[4]]){//09, Bowie & Dick
         ...
         if(RX_Buffer[4]==0x09 && RX_Buffer[5]==0x0f && estados[19+RX_Buffer[4]]){
            printf("page Funcionamiento");
            SendDataDisplay();       
            ConfiguraCiclo(30,3,0,1,0,9,134,7,25,0);  
            Op=OFF;
            Op2=OFF;
            MenuAnt=Menu;
            //TiempoVacio=CantidadPulsos*180;
      ...
      if(RX_Buffer[4]==0x0a && RX_Buffer[5]==0x0f && estados[19+RX_Buffer[4]]){//0a, Test de Fugas
         ...
         if(RX_Buffer[4]==0x0a && RX_Buffer[5]==0x0f && estados[19+RX_Buffer[4]]){
            printf("page Funcionamiento");
            SendDataDisplay();       
            ConfiguraCiclo(0,20,0,0,0,10,30,1,17,1);
            MenuAnt=Menu;
            //TiempoVacio=CantidadPulsos*180;
         }
      }
...
//_--------------------------------------------------------------------------------------------------------------------------------------------------------      
    if(Menu==6){ // Menu de Prueba de Componentes
         ...
         if(RX_Buffer[4]==0x0b){//Oprimio SSR1
            EstadoSSR1=!EstadoSSR1;
            RX_Buffer[4]=0x00;
            RX_Buffer2[4]=0x00;
         }
         ...
         if(RX_Buffer[4]==0x0c){//Oprimio SSR2
         ...
         if(RX_Buffer[4]==0x0d){//Oprimio Aux
            EstadoAux=!EstadoAux;
            RX_Buffer[4]=0x00;
            RX_Buffer2[4]=0x00;
         }
...
//_--------------------------------------------------------------------------------------------------------------------------------------------------------    
   ...
   if(PresionPreCamara>280 || PresionCamara>360){
      AlarmaPresion=ON;
   }
   ...
   if(AlarmaPuerta || AlarmaTermostato || AlarmaSobreTemperatura || AlarmaTiempoProlongado || AlarmaEmergencia || AlarmaPresion || AlarmaBomba 
         || AlarmaVacio || AlarmaLlenado || AlarmaEmpaque){
      ActivaAlarma(1);
      if(AlarmaEmergencia){
         codigoAlarma=1;
         ...
         Finalizo=ON;
      }else if(AlarmaPuerta && !AlarmaEmpaque){
         codigoAlarma=2;
         ...
         if(!PRES)
            Desfogue_on;
         if(AlarmaEmpaque && PresionCamara<Atmosferica+5){
            Empaque_off;
         }else{
         ...
         Finalizo=ON;
      }else if(AlarmaEmpaque){ // Alarma de Error de que el empaque no se presuriza
         codigoAlarma=30;
         printf("Funcionamiento.t4f.txt=\" Error Empaque\"");
         SendDataDisplay();
         printf("Funcionamiento.t5f.txt=\" Cancelado\"");
         SendDataDisplay();
         if(AlarmaEmpaque && PresionCamara<Atmosferica+4)
            Empaque_off;
         Desfogue_on;
         Paso_off;
         if(!abrirPuerta)
      ...
      if(!CalderinLleno){
         ...
         if(!Vacio){ //Si no ha realizado los pulsos de vacio
            ...
            if(PulsoPositivo==5){//Pulso Positivo
               ...
               Paso_on;     
               Agua2_off;
               if(PresionCamara>=Atmosferica+80){
                  Paso_off;
                  PulsoPositivo=10;
               }  
            }
   ...
   {                      
      ...
      printf("fecha.txt=\"%s  %02u/%02u/20%02u   %02u:%02u\"",usuario,Dia,Mes,Year,Hora,Minuto);
      SendDataDisplay();
   }
