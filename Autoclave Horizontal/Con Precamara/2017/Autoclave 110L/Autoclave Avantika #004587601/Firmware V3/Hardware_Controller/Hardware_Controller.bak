...
int8 r=0,t=0,r2=0,t2=0,TiempoControl=0,z=0,Frio=0;
...
float Leer_Sensor(int media)
{
   ...
   V0 = V0/39.8; // Se elimina la ganancia
...
//_--------------------------------------------------------------------------------------------------------------------------------------------------------    
   ...
   {      
      ...
      if(Control==1 && !DOOR && !STOP && AlarmaAgua==0 && Alarma==0) // Si esta en condiciones normales para ejecutar el control.
      {
         ...
         if(PresionPreCamara>=136) // Si el Presostato se activo indicando que el calderin alcanzo la presión ajustada.
         {  
            if(flag_presostato==0)
            {
               delay_ms(500);
               if(PresionPreCamara>=136)
               {
