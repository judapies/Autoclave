...
unsigned int proceso=0, minutos=0, minutos2=0;
...
delay_us(10);
  ...
  lcd_enviar(lcd_comando,lcd_clear);
   ...
   minutos2=read_eeprom(20);
   delay_ms(1000);
   
   if(!UP&&!RIGHT)
   {printf(lcd_putc,"\f");delay_ms(1000);
      for(;;)
      {
      
         if(!UP)
         {minutos2++;delay_ms(40);write_eeprom(20,minutos2);}
         if(!DOWN)
         {minutos2--;delay_ms(40);write_eeprom(20,minutos2);}
         lcd_gotoxy(3,1);
         printf(lcd_putc,"Tiempo de Rampa");
         lcd_gotoxy(3,3);
         printf(lcd_putc,"Minutos:%i",minutos2);
...
//
      for(minutos=0;minutos<minutos2;minutos++) // Especifica el tiempo total de la rampa
