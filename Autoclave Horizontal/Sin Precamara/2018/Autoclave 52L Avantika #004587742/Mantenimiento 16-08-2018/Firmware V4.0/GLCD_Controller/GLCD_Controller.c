...
void VerifiqueAgua(void){
   glcd_fillscreen(OFF);
   strcpy(Menus.Letra,"  !Verifique que el  ");
   displayMenu(Menus.Letra,0,15,0,1);
   strcpy(Menus.Letra,"   reservorio tenga  ");
   displayMenu(Menus.Letra,0,30,0,1);
   strcpy(Menus.Letra,"   agua suficiente!  ");
   ...
   else
   {
      IniciaCiclo=ON;
         if(!CalderinLleno)
         {
            ...
            Desfogue_on; 
            if(!LOW && PresionCamara<5){//Si no sensa agua en el nivel bajo
               AguaIn_on;
               IniciaLlenado=ON;
            }else{
               if(!HIGH){//Si no sensa agua en el nivel alto y no se ha llenado el calderin
                  if(PresionCamara<5){
                     AguaIn_on;
                     IniciaLlenado=ON;
                  }
