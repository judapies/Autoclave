...
short SensadoNivelL=OFF,NivelBajo=OFF,onBomba=OFF,flagBombaAgua=OFF,flagBombaAguaOff=OFF,calientaSecado=OFF;
...
int16 tbuzzer=0,tsilencio=0,tSecado=0;
...
void main(){
   ...
   ApagaSalidas();
   TiempoVacio=360; 
   Sensor[0].Gain=214.61;
   Sensor[1].Gain=214.61;
   Imprime=10;
      ...
      if(RX_Buffer[3]==0x0f || UP){//0f, recibe caracteres ingresados desde el Display
         ...
         if(RX_Buffer[3]==0x0f || UP){
            ...
            if(Opcion==10){// Cambio de Clave
               ...
               SendDataDisplay();       
               ConfiguraCiclo(30,3,0,2,0,5,134,2,25,0);
               MenuAnt=Menu;     
               ...
               printf("page Funcionamiento");
               SendDataDisplay();       
               ConfiguraCiclo(0,20,0,20,0,4,121,2,25,0);
               MenuAnt=Menu;     
            }else if(Opcion==3){//Delicado no envuelto
               printf("page Funcionamiento");
               SendDataDisplay();       
               ConfiguraCiclo(0,20,0,1,0,3,121,2,25,0);
               MenuAnt=Menu;     
            }else if(Opcion==2){//Instrumental Envuelto
               printf("page Funcionamiento");
               SendDataDisplay();       
               ConfiguraCiclo(0,4,0,20,0,2,134,2,25,0);
               MenuAnt=Menu;               
            }else if(Opcion==1){//Instrumental no Envuelto
               printf("page Funcionamiento");
               SendDataDisplay();       
               ConfiguraCiclo(0,4,0,1,0,1,134,2,25,0);               
               MenuAnt=Menu;
            }
         }
      }else{
                  ...
                  if(desfoguelento){
                     ...
                     if(PresionCamara<=(Atmosferica+10)){  
                        if(sSec==0 && mSec==0){                          
                           ...
                           if(!DOOR && tSecado<1200){
                              calientaSecado=ON;
                           }else{
                              calientaSecado=OFF;
                           }
                           
                           if(calientaSecado && tSecado<1200){
                              SSR2_on; 
                           }
                           if(tSecado>=1200){
                              calientaSecado=OFF;
                           ...
                           if(PresionCamara>NivelVacio){//Si la presión de la camara es mayor al nivel de vacio
                              salidasPulsoNegativo(); 
                              Agua_off;  
                              //EV8_off;
                              apagaBombaAgua();
                           ...
                           if(PresionCamara<=(NivelVacio-5)){//Si alcanzo el nivel de vacio requerido
                              codigoAlarma=15;
                              if(codigoAnt!=codigoAlarma){
                                 codigoAnt=codigoAlarma;
                                 GuardaDatoCiclo();                  
