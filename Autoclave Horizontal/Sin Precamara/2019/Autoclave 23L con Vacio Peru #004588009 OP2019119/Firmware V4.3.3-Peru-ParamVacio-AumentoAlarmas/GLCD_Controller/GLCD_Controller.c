...
int8 Setpoint=0,Opcion=1,Opcion2=1,tiempo_esterilizacion=0,tiempo_secado=0,tiempoImpresion=0,vacioTest=21,vacioCiclo=25,pulsosConfigurados=2;
      ...
      if(RX_Buffer[3]==0x0f || UP){//0f, recibe caracteres ingresados desde el Display
         ...
         if(RX_Buffer[3]==0x0f || UP){
            ...
            if(Opcion==10){// Cambio de Clave
               ...
               printf("page Funcionamiento");
               SendDataDisplay();       
               ConfiguraCiclo(0,20,0,0,0,6,30,1,vacioTest,1);
               MenuAnt=Menu;     
   ...
   if(AlarmaPuerta || AlarmaTermostato || AlarmaSobreTemperatura || AlarmaTiempoProlongado || AlarmaEmergencia || AlarmaPresion || AlarmaBomba 
      ...
      if(!CalderinLleno){
         ...
         if(!Vacio){ //Si no ha realizado los pulsos de vacio 
            ...
            if(tvacio>900)//Si excede el tiempo maximo para generar los pulsos de vacio genera alarma            
               AlarmaVacio=ON;            
         }else{
            IniciaVacio=OFF;          
            if(!Esterilizando){
