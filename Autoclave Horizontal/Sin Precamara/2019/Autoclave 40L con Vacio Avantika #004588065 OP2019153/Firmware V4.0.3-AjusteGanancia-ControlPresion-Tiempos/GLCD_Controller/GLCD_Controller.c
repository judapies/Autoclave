...
//Tiempos//---------------------
int16 TiempoCiclo=0,TiempoLlenado=0,TiempoSensadoNivel=0,Tp=0,Tc=0,tBombaAgua=0,tBombaAguaOff=0,tStop=0,tTermostato=0;
...
int8 h=0,l=0,Ajuste=0,Ajusten=0,MediaMovil=20,PantallaPrincipal=0,TipoClave=0,Modulo=0,Digito=0,Tapertura=0,yy=0;
float V0=0.0,Pt,Tciclo=10.0,Tpulso=0.0,error=0.0,Prom=0.0,Temperature=0.0,tmp=0.0,Ganancia=25.0,GananciaIn=25.0,Ganancia2=20.0,desvio=1.0,tmp2=0.0;
...
short desfoguelento=0,Lectura=0,Grafica=0,flagG=0;
...
int8 iconoGrande[10]={22,62,25,24,58,64,23,28,20,46};
int8 iconoPeque[10]={18,61,34,33,57,63,32,38,19,17};
char const texto[10][20]={"NO ENVUELTO","ENVUELTO","LIQUIDOS A","LIQUIDOS B","BOWIE & DICK","TEST VACIO","LIBRE","TEST COMP.","HORA Y FECHA","CLAVE"};
...
void main(){
   ...
   ApagaSalidas();
   TiempoVacio=360; 
   Sensor[0].Gain=104.34;
   Sensor[1].Gain=104.34;
   Imprime=10;
      ...
      if(RX_Buffer[3]==0x0f || UP){//0f, recibe caracteres ingresados desde el Display
         ...
         if(RX_Buffer[3]==0x0f || UP){
            ...
            if(Opcion==10){// Cambio de Clave
               ...
               SendDataDisplay();       
               ConfiguraCiclo(30,3,0,2,0,5,134,5,25,0);
               MenuAnt=Menu;     
               ...
               MenuAnt=Menu;     
            }else if(Opcion==4){//Liquidos B
               printf("page Funcionamiento");
               SendDataDisplay();       
               ConfiguraCiclo(0,20,0,0,1,4,121,0,28,0);
               MenuAnt=Menu;     
            }else if(Opcion==3){//Liquidos A
               printf("page Funcionamiento");
               SendDataDisplay();       
               ConfiguraCiclo(0,15,0,0,1,3,121,0,28,0);
               MenuAnt=Menu;     
            }else if(Opcion==2){//Instrumental Envuelto
               printf("page Funcionamiento");
               SendDataDisplay();       
               ConfiguraCiclo(0,4,0,20,0,2,134,4,28,0);
               MenuAnt=Menu;               
            }else if(Opcion==1){//Instrumental no Envuelto
               printf("page Funcionamiento");
               SendDataDisplay();       
               ConfiguraCiclo(0,4,0,1,0,1,134,4,28,0);               
               MenuAnt=Menu;
            }
         }
      }else{
   ...
   if(Menu==8){ //Menu Ciclos Personalizados
      ...
      if(RX_Buffer[4]>0x00 && RX_Buffer[4]<0x04){//Personalizados
         ...
         if(RX_Buffer[4]<3){
            ...
            printf("tsec.txt=\"00:00\"");
            SendDataDisplay();
            printf("pulsos.txt=\"0\"");
            SendDataDisplay();
            printf("vacio.txt=\"--- kPa\"");
            SendDataDisplay();
         }
         ...
         printf("vacio.txt=\"---\"");
         SendDataDisplay();
      }else if(CicloLibre>0 && CicloLibre<3){
      ...
      if(RX_Buffer[4]==0x40 && (CicloLibre>0 && CicloLibre<3)){//30, Tsec
      ...
      if(RX_Buffer[4]==0x50 && (CicloLibre>0 && CicloLibre<3)){//50, PulsosVacio
         printf("page Pulsos");
         SendDataDisplay();     
         Pulsos=CiclosPersonalizados[CicloLibre-1].PulsosVacio;
      }
      
      if(RX_Buffer[4]==0x80 && (CicloLibre>0 && CicloLibre<3)){//80, Nivel Vacio
         printf("page Nivel");
         SendDataDisplay();     
      ...
      if(RX_Buffer[4]==0x70 && CicloLibre>0){//70, Iniciar Ciclo         
         ...
         if(CicloLibre<3){
            ...
            ConfiguraCiclo(CiclosPersonalizados[CicloLibre-1].Segundoest,CiclosPersonalizados[CicloLibre-1].Minutoest,0,0,1,CicloLibre+6,
            CiclosPersonalizados[CicloLibre-1].Temperatura,0,25,0);
         }
      }
   }
//_--------------------------------------------------------------------------------------------------------------------------------------------------------       
