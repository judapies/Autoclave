...
short flagBuzzer=OFF,flagSilencio=OFF,Finalizo=OFF,AlarmaPresion=OFF,Libre=OFF,purga=OFF,entro=OFF,Aire=OFF,flagBombaAgua=OFF,flagBombaAguaOff=OFF;
...
int8 iconoGrande[15]={22,60,68,59,25,24,66,64,16,62,23,28,20,46};
int8 iconoPeque[15]={18,57,67,58,34,33,65,63,36,61,32,38,19,17};
char const texto[14][20]={"NO ENVUELTO","ENVUELTO","DEL. NO ENVUELTO","DEL. ENVUELTO","LIQUIDOS A","LIQUIDOS B","LIQUIDOS C","SOLIDOS A","SOLIDOS B","DESINFECCION","LIBRE","TEST","HORA Y FECHA","CLAVE"};
...
Personalizado CiclosPersonalizados[10];
      ...
      if(RX_Buffer[3]==0x0f || UP){//0f, recibe caracteres ingresados desde el Display
         ...
         if(PantallaPrincipal==0){//Si esta en menu principal
            ...
            if(Opcion==14){// Cambio de Clave
               ...
               RX_Buffer2[3]=0x00;
               Opcion=0;
            }else if(Opcion==13){//Hora y Fecha
               printf("page Fecha");
               SendDataDisplay();     
               PantallaPrincipal=0;
               MenuAnt=Menu;
               ...
               Op=OFF;
               Op2=OFF;
            }else if(Opcion==11){//Ciclo Libre
               printf("page CicloPersona");
               SendDataDisplay(); 
               MenuAnt=Menu;
               Op=OFF;
               Op2=OFF;
            }else if(Opcion==10){//Desinfeccion
               printf("page Funcionamiento");
               SendDataDisplay();       
               ConfiguraCiclo(0,0,5,2,0,0,5,1,1,10,105);
               Op=OFF;
               Op2=OFF;
               MenuAnt=2;
            }else if(Opcion==9){//SolidosB
               printf("page Funcionamiento");
               SendDataDisplay();       
               ConfiguraCiclo(0,0,4,0,0,0,0,3,0,9,134);
               Op=OFF;
               Op2=OFF;
               MenuAnt=2;
            }else if(Opcion==8){//SolidosA
               printf("page Funcionamiento");
               SendDataDisplay();       
               ConfiguraCiclo(0,0,5,1,0,0,0,3,0,8,121);
               Op=OFF;
               Op2=OFF;
               MenuAnt=2;
            }else if(Opcion==7){//LiquidosC
               printf("page Nivel");
               SendDataDisplay();       
               ConfiguraCiclo(0,0,0,3,0,0,0,0,1,7,121);
               Op=OFF;
               Op2=OFF;
               MenuAnt=2;
            }else if(Opcion==6){//LiquidosB
               printf("page Nivel");
               SendDataDisplay();       
               ConfiguraCiclo(0,0,0,2,0,0,0,0,1,6,121);
               Op=OFF;
               Op2=OFF;
               MenuAnt=2;
            }else if(Opcion==5){//LiquidosA
               printf("page Nivel");
               SendDataDisplay();       
               ConfiguraCiclo(0,0,5,1,0,0,0,0,1,5,121);
               Op=OFF;
               Op2=OFF;
               MenuAnt=2;
               ...
               printf("page Funcionamiento");
               SendDataDisplay();       
               ConfiguraCiclo(0,0,0,3,0,0,0,2,0,4,121);
               Op=OFF;
               Op2=OFF;
               MenuAnt=2;
               ...
               printf("page Funcionamiento");
               SendDataDisplay();       
               ConfiguraCiclo(0,0,5,1,0,0,5,1,0,3,121);
               Op=OFF;
               Op2=OFF;
               MenuAnt=2;
            }else if(Opcion==2){//Envuelto
               printf("page Funcionamiento");
               SendDataDisplay();       
               ConfiguraCiclo(0,0,2,1,0,0,0,3,0,2,134);
               Op=OFF;
               Op2=OFF;
               MenuAnt=Menu;               
            }else if(Opcion==1){//No Envuelto
               printf("page Funcionamiento");
               SendDataDisplay();       
               ConfiguraCiclo(0,0,4,0,0,0,5,1,0,1,134);
               Op=OFF;
               Op2=OFF;
               MenuAnt=Menu;
            }
...
//_--------------------------------------------------------------------------------------------------------------------------------------------------------
   if(Menu==8){ //Menu Ciclos Personalizados
      ...
      if(RX_Buffer[4]>0x00 && RX_Buffer[4]<0x07){//Personalizados
         ...
         printf("desfogue.txt=\"--\"");
         SendDataDisplay();
      }else if(CicloLibre>0 && CicloLibre<7){
   ...
   || AlarmaCiclo){
      ...
      if(flagImprime && ActivaImpresion==1){
         ...
         if(!CalderinLleno){
            ...
            Agua_off;
            if(Temperatura>=((float)Setpoint-0.3)){// || Tactual>=((float)Setpoint-0.3)){
               Esterilizando=ON;
            }
