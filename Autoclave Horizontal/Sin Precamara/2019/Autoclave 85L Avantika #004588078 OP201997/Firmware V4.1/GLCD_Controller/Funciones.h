...
float LeerPT100(int media)
{
   ...
   V0 = V0/104.48; // Se elimina la ganancia
   //V0 += 14.82;
   V0 += 14.83;
   Pt=R10/((1023/V0)-1);
...
void ControlaPresion(){
   ...
   Tactual=((PresionCamara+72)*(PresionCamara+72)*p1)+((PresionCamara+72)*p2)+p3;
//-------------------------------Control de Calentamiento------------------------------------------------------------//         
   if(Esterilizando){
...
//Lectura EEPROM//--------------------------
   ...
   for(i=0;i<10;i++){//Lee valores almacenados de ciclos personalizados
