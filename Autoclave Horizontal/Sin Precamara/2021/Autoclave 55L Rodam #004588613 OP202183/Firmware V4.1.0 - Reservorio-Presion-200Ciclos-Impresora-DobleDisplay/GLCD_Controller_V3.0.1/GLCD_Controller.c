...
#define Absoluta //Define para el sensor de presion Absoluta
#define Tmax 128
#define Tmin 105
#define ULKA
//--------------------------------------------------------------------------//
...
//Alarmas//---------------------
short AlarmaTiempoProlongado=OFF,AlarmaSobreTemperatura=OFF,AlarmaLlenado=OFF,AlarmaTermostato=OFF,AlarmaPuerta=OFF,AlarmaEmergencia=OFF,AlarmaCiclo=OFF,AlarmaPuertaL=OFF,borra=OFF;

//Banderas//--------------------
short IniciaLlenado=OFF,CalderinLleno=OFF,IniciaCiclo=OFF,Esterilizando=OFF,Secando=OFF,SensadoNivel=OFF,Desfogando=OFF,DesfogueSuave=OFF,LOW=OFF,HIG=OFF;
...
//Tiempos//---------------------
int16 TiempoCiclo=0,TiempoLlenado=0,TiempoSensadoNivel=0,Tp=0,Tc=0,tguarda=0;
...
int8 h=0,l=0,Ajuste=0,Ajusten=0,MediaMovil=20,TTrampa=0,PantallaPrincipal=0,TipoClave=0,Modulo=0,Digito=0,yy=0,codigoAlarma=0,codigoAnt=0;
int16 posicionDato=0,cicloVisto=0,cicloVisualizado=0;
...
int8 Menu=0, MenuAnt=0,MenuAntt=240,CiclosL=0,CiclosH=0,CicloSeleccionado=0;
int8 Setpoint=0,Opcion=1,Opcion2=1,tiempo_esterilizacion=0,tiempo_secado=0,tiempoImpresion=0;
int16 tiempos=0,tiempos2=0,tiempos3=0,tiempoctrl=0,Nivel=0,tinicio=0,PresionCamara=0,PresionPreCamara=0,Ciclos=0,NivelSeleccionado=0,tBombaAgua=0,tBombaAguaOff=0,i=0,j=0;
...
int8 Year=18,Mes=9,Dia=13,Hora=0,Minuto=0,Segundo=0,dow=0,mEst=0,sEst=0,mSec=0,sSec=0;
...
void main()
{
   ...
   #if defined(PT100Fija) && !defined(Absoluta)
      Atmosferica=0;
   #else
      if(!DOOR){
   ...
   while(true)
   {
      ...
      Envio_Esclavos();
      delay_ms(1);
      sEst=Testerilizacion[0]+(Testerilizacion[1]*10);
      mEst=Testerilizacion[2]+(Testerilizacion[3]*10);
      sSec=Tsecado[0]+(Tsecado[1]*10);
      mSec=Tsecado[2]+(Tsecado[3]*10);
      //Lectura_Esclavos();
      restart_wdt();
...
// Tiempo de Muestreo//--------------------------------------
      if(tiempos3>=20){
         ...
         PresionPreCamara=0;//Leer_Sensor_Presion_PreCamara(10);
...
//_--------------------------------------------------------------------------------------------------------------------------------------------------------      
   if(Menu==12){//Menu de Visualizacion de Ciclos Realizados
      ...
      */      
      ...
      printf("t0.txt=\"Ciclo %02Lu\"",cicloVisto);
      SendDataDisplay();
...
//_--------------------------------------------------------------------------------------------------------------------------------------------------------    
   ...
   if(AlarmaPuerta || AlarmaPuertaL || AlarmaTermostato || AlarmaLlenado || AlarmaSobreTemperatura || AlarmaTiempoProlongado || AlarmaEmergencia || AlarmaPresion
   || AlarmaCiclo){
      ActivaAlarma(1);
      if(AlarmaEmergencia){
         ...
         if(!flagImprimir){
            ...
            codigoAlarma=2;
            printf("Funcionamiento.t4f.txt=\" Puerta Lado Sucio Abierta\"");
            SendDataDisplay();
      ...
      }else if(AlarmaPuertaL){
            codigoAlarma=9;
            printf("Funcionamiento.t4f.txt=\" Puerta Lado Limpio Abierta\"");
            SendDataDisplay();
            printf("Funcionamiento.t5f.txt=\" Pausado\"");
            SendDataDisplay();
            SSR_off;
                  ...
                  if(desfoguelento){
                     ...
                     if(PresionCamara<=Atmosferica+25){
                        ...
                        if(PresionCamara<Atmosferica+2){
                           ...
                           if(Temperatura<=90){
                              ActivaAlarma(2);
                              printf("Funcionamiento.t4f.txt=\" Abra Lado Limpio\"");
                              SendDataDisplay();
