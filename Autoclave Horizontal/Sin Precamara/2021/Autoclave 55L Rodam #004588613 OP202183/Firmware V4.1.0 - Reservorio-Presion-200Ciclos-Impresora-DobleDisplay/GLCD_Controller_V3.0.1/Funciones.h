...
//-------------------------------Control de Calentamiento------------------------------------------------------------//         
   ...
   if(error>1.0 && error<30 && Aumento<0.3){
      if(Tpulso<6.0){
         flagTC=1;
...
void Carga_Vector(void){
   ...
   SlaveA0Tx[28]=sSec;
   SlaveA0Tx[29]=Menu;
   SlaveA0Tx[30]=Setpoint;
   SlaveA0Tx[31]=codigoAlarma;
   SlaveA0Tx[32]=255;
   SlaveA0Tx[33]=128;
   SlaveA0Tx[34]=50;
}
...
void LimitaValores(){
   ...
   if(MediaMovil>80 || MediaMovil<20){
      MediaMovil=80;
      write_eeprom(24,80);
      delay_ms(10);
   }
...
void codificaAlarma(){
   if(ACiclos[i]==0){
      ...
      printf("A%Lu.txt=\"ERR Ciclo\"",i+1);
      SendDataDisplay();
   }else if(ACiclos[i]==8){
      printf("A%Lu.txt=\"Llenado\"",i+1);
      SendDataDisplay();
   }else if(ACiclos[i]==9){
      printf("A%Lu.txt=\"Puerta L\"",i+1);
      SendDataDisplay();
   }else if(ACiclos[i]==10){
      printf("A%Lu.txt=\"Llenando\"",i+1);
      SendDataDisplay();
...
void ConfiguraCiclo(int8 este0,este1,este2,este3,secado0,secado1,secado2,secado3,liquidos,ciclo,temperatura){
   ...
   sEst=este0+(este1*10);
   mEst=este2+(este3*10);
   sSec=secado0+(secado1*10);
   mSec=secado2+(secado3*10);
   if(!borra){
      borraDatoCiclo();
      borra=ON;
   }
