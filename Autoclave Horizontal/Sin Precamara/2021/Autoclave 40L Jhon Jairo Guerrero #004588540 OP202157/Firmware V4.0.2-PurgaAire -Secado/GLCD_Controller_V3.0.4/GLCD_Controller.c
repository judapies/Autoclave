...
#define   SSR2_on          output_bit(PIN_A3,1)
#define   SSR2_off         output_bit(PIN_A3,0)
...
short EstadoDesfogue=OFF,EstadoAgua=OFF,EstadoSSR=OFF,PruebaEstados=OFF,Ciclo=OFF,flagCalentando=OFF,flagEsterilizando=OFF,EstadoAire=OFF,EstadoSSR2=OFF;
...
short flagBuzzer=OFF,flagSilencio=OFF,Finalizo=OFF,AlarmaPresion=OFF,Libre=OFF,purga=OFF,entro=OFF,Aire=OFF,purgo=OFF,purgaAire2=OFF;

//Tiempos//---------------------
int16 TiempoCiclo=0,TiempoLlenado=0,TiempoSensadoNivel=0,Tp=0,Tc=0,tsecados=800;
...
int16 tiempos=0,tiempos2=0,tiempos3=0,tiempoctrl=0,Nivel=0,tinicio=0,PresionCamara=0,PresionPreCamara=0,Ciclos=0,NivelSeleccionado=0,tPurgaAire=0,tStop=0,tTermostato=0;
signed int  Contrasena[4]={0,0,0,0};      // Contrasena de 4 digitos
signed int  Password[4]={0,0,0,0};        // Contrasena Almacenada de 4 digitos
int8 r=0,t=0,r22=0,t2=0,TiempoControl=0;
int8 y=0,ConteoBuzzer=0;
int16 t_exhaust=0,tbuzzer=0,tsilencio=0,tiempoAgua=0,tAire=0;
...
//_--------------------------------------------------------------------------------------------------------------------------------------------------------      
      if(Menu==6){ // Menu de Prueba de Componentes
         ...
         if(RX_Buffer[4]==0x06)//Oprimio SSR2
         {
...
//_--------------------------------------------------------------------------------------------------------------------------------------------------------    
   ...
   if(tTermostato>=2000 && !Secando && !Finalizo){
      AlarmaTermostato=ON;
   }
      ...
      if(RX_Buffer[4]==0x11 || (RIGHT && LEFT)){//Tecla Regresar
         ...
         if(RX_Buffer[4]==0x11 || (RIGHT && LEFT)){
            if(!GuardaEEPROM){
               ...
               delay_ms(10);
               write_eeprom(46,make8(tsecados,0));
               delay_ms(10);
               write_eeprom(47,make8(tsecados,1));
               delay_ms(10);
               GuardaEEPROM=ON;
            }
            printf("page MenuPrincipal");
