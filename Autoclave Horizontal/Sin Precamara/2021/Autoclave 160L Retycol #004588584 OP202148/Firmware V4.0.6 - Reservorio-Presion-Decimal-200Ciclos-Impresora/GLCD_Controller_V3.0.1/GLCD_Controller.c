...
//Tiempos//---------------------
int16 TiempoCiclo=0,TiempoLlenado=0,TiempoSensadoNivel=0,Tp=0,Tc=0,tguarda=0,tsecados=50;
                     ...
                     if(PresionCamara<=Atmosferica+25){
                        ...
                        if(PresionCamara<Atmosferica+2){
                           if(!DOOR){
                              ...
                              Desfogue_on;
                              if(Temperatura<90 && PresionCamara<=1){
                                 CalientaChaqueta(50);
                              }else{
                                 Controlando=0;
                                 Control=0;
...
//_--------------------------------------------------------------------------------------------------------------------------------------------------------
      ...
      if(Opcion2==1){
         ...
         printf("Config.t2c.txt=\"Secado\"");
         SendDataDisplay();      
         printf("t3c.txt=\"%04Lu\"",tsecados);
         SendDataDisplay();
         if(RX_Buffer[4]==0x0a || UP){//Tecla Arriba Oprimida
            delay_ms(20);
            if(RX_Buffer[4]==0x0a || UP){
      ...
      if(RX_Buffer[4]==0x11 || (RIGHT && LEFT)){//Tecla Regresar
         ...
         if(RX_Buffer[4]==0x11 || (RIGHT && LEFT)){
            if(!GuardaEEPROM){
               ...
               delay_ms(10);
               write_eeprom(47,make8(tsecados,0));
               delay_ms(10);
               write_eeprom(48,make8(tsecados,1));
               delay_ms(10);
               GuardaEEPROM=ON;
            }
            printf("page MenuPrincipal");
