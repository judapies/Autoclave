...
//_--------------------------------------------------------------------------------------------------------------------------------------------------------
      ...
      if(visual==1)
      {
         ...
         if(Opcion<=6)
         {
            ...
            if(Opcion==6)   
               {strcpy(Menus.Letra,"Desinfeccion");
         ...
         else
         {
            ...
            if(Opcion==7)   
               {strcpy(Menus.Letra,"Delicado Embalado");
               displayMenu(Menus.Letra,0,10,1,1);}
            else
               {strcpy(Menus.Letra,"Delicado Embalado");
               displayMenu(Menus.Letra,0,10,0,1);}
            ...
            if(Opcion==8)   
               {strcpy(Menus.Letra,"Delicado NO Embalado");
               displayMenu(Menus.Letra,0,19,1,1);}
            else
               {strcpy(Menus.Letra,"Delicado NO Embalado");
               displayMenu(Menus.Letra,0,19,0,1);}   
...
//_--------------------------------------------------------------------------------------------------------------------------------------------------------      
      ...
      if(RIGHT)// Si oprime derecha
      {
         ...
         Tsecadop[0]=0;Tsecadop[1]=0;Tsecadop[2]=0;Tsecadop[3]=0;Setpoint=121;Menu=18;delay_ms(700);glcd_fillScreen(OFF);    
      }
    }    
//_--------------------------------------------------------------------------------------------------------------------------------------------------------    
...
//_--------------------------------------------------------------------------------------------------------------------------------------------------------    
 ...
 //_--------------------------------------------------------------------------------------------------------------------------------------------------------
      ...
      if(visual==1)
      {
         ...
         strcpy(Menus.Letra,"T. Esterilizacion:15m");
         displayMenu(Menus.Letra,0,36,0,1);
         strcpy(Menus.Letra,"T. Secado= 30m");
         displayMenu(Menus.Letra,0,46,0,1);           
         visual=0;
      }
      ...
      if(RIGHT)// Si oprime derecha
      {
         ...
         Tsecadop[0]=0;Tsecadop[1]=0;Tsecadop[2]=0;Tsecadop[3]=3;Setpoint=121;Menu=20;delay_ms(700);
         Menu=20;glcd_rect(15, 18, 30, 53, NO, ON);tiempo_secado=0;enable_interrupts(global);glcd_fillScreen(OFF);
         strcpy(Menus.Letra,"Calentando");
         displayMenu(Menus.Letra,0,55,2,1);
         glcd_rect(15, 18, 30, 53, NO, ON);
...
//_--------------------------------------------------------------------------------------------------------------------------------------------------------
      if(Menu==9){ // Menu de Desinfeccion
      ...
      {
         Testerilizacionp[0]=0;Testerilizacionp[1]=0;Testerilizacionp[2]=5;Testerilizacionp[3]=2;visual=1;
         Tsecadop[0]=0;Tsecadop[1]=0;Tsecadop[2]=5;Tsecadop[3]=1;Setpoint=105;Menu=20;delay_ms(700);
         Menu=20;glcd_rect(15, 18, 30, 53, NO, ON);tiempo_secado=0;enable_interrupts(global);glcd_fillScreen(OFF);
         strcpy(Menus.Letra,"Calentando");
         displayMenu(Menus.Letra,0,55,2,1);
         glcd_rect(15, 18, 30, 53, NO, ON);
...
//_--------------------------------------------------------------------------------------------------------------------------------------------------------
      if(Menu==10){ // Menu de Delicado Embalado
      
      if(visual==1)
      {
         glcd_imagen(1); //Tijeras y Camisa
         strcpy(Menus.Letra,"Del. Emb");
         displayMenu(Menus.Letra,0,0,1,2);
      
         /*strcpy(Menus.Letra,"Del. Embalado");
         displayMenu(Menus.Letra,0,55,2,1);*/
         
         strcpy(Menus.Letra,"Temperatura:121 C");
         displayMenu(Menus.Letra,0,26,0,1);
         strcpy(Menus.Letra,"T. Esterilizacion:30m");
         displayMenu(Menus.Letra,0,36,0,1);
         strcpy(Menus.Letra,"T. Secado= 30m");
         displayMenu(Menus.Letra,0,46,0,1);           
         visual=0;
      ...
      if(RIGHT)// Si oprime derecha
      {
         Testerilizacionp[0]=0;Testerilizacionp[1]=0;Testerilizacionp[2]=0;Testerilizacionp[3]=3;visual=1;
         Tsecadop[0]=0;Tsecadop[1]=0;Tsecadop[2]=0;Tsecadop[3]=3;Setpoint=121;Menu=20;delay_ms(700);
         Menu=20;glcd_rect(15, 18, 30, 53, NO, ON);tiempo_secado=0;enable_interrupts(global);glcd_fillScreen(OFF);
         strcpy(Menus.Letra,"Calentando");
         displayMenu(Menus.Letra,0,55,2,1);
...
//_--------------------------------------------------------------------------------------------------------------------------------------------------------
      if(Menu==11){ // Menu de Delicado NO Embalado
      
      if(visual==1)
      {
         glcd_imagen(1); //Tijeras y Camisa
         strcpy(Menus.Letra,"Del NO Emb");
         displayMenu(Menus.Letra,0,0,1,2);
      
         /*strcpy(Menus.Letra,"Del. NO Embalado");
         displayMenu(Menus.Letra,0,55,2,1);*/
...
//_--------------------------------------------------------------------------------------------------------------------------------------------------------
      ...
      if(RIGHT)// Si oprime derecha
      {
         Testerilizacionp[0]=0;Testerilizacionp[1]=0;Testerilizacionp[2]=4;Testerilizacionp[3]=0;visual=1;
         Tsecadop[0]=0;Tsecadop[1]=0;Tsecadop[2]=0;Tsecadop[3]=3;Setpoint=121;Menu=20;delay_ms(700);
         Menu=20;glcd_rect(15, 18, 30, 53, NO, ON);tiempo_secado=0;enable_interrupts(global);glcd_fillScreen(OFF);
         strcpy(Menus.Letra,"Calentando");
         displayMenu(Menus.Letra,0,55,2,1);
...
//_--------------------------------------------------------------------------------------------------------------------------------------------------------      
      if(Menu==13){ // Menu de Proceso Personalizado
         ...
         if(RIGHT)//Si oprime SET
         {  if(Flanco1 == 0) 
               {Flanco1 = 1;delay_ms(300);Menu=Opcion+14;Opcion=1;glcd_fillscreen(OFF);visual=1;
               }
         }
            else
               {Flanco1 = 0;}  
...
//_--------------------------------------------------------------------------------------------------------------------------------------------------------      
      if(Menu==14){ // Menu de Cambio de Contraseña
...
//_--------------------------------------------------------------------------------------------------------------------------------------------------------      
      if(Menu==15){ // Menu de Tiempo de Esterilización
      ...
      if(visual==1)
      {
         ...
         if(LEFT)//Si oprime boton de Toma.
         {
            delay_ms(700);Menu=13;glcd_fillscreen(OFF);Opcion=1;visual=1;
...
//_--------------------------------------------------------------------------------------------------------------------------------------------------------      
      if(Menu==16){ // Menu de Tiempo de Secado
...
//_--------------------------------------------------------------------------------------------------------------------------------------------------------      
      if(Menu==17){ // Menu de Temperatura
