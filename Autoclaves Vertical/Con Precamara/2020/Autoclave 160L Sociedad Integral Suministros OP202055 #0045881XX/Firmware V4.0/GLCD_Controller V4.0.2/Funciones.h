...
float LeerPT100(int8 canal,int media)
{
   ...
   Sensor[canal].V0 = Sensor[canal].V0+14.79; // Se suma voltaje de divisor fijo
...
float Leer_Sensor_Presion_PreCamara(int media){
   ...
   //Punto cero con esta ecuacion es 41 cuentas del ADC
   Presionp=(((V2)/1023)-0.04)/K;//K=0.0012858
...
//Lectura EEPROM//--------------------------
   ...
   delay_ms(10);
   Integral=read_eeprom(27);
   delay_ms(10);
   Derivativo=read_eeprom(28);
   delay_ms(10);
...
void ControlaPresion(){
   if(Setpoint<125){
      if(Setpoint<115){
         if(PresionPrecamara>=60){
            SSR_off;
            Precalentamiento=ON;
         }
         if(PresionPrecamara<50)
            SSR_on;
      }else{ 
         if(PresionPrecamara>=140){
            SSR_off;
            Precalentamiento=ON;
         }
...
void codificaAlarma(){
   if(ACiclos[i]==0){
      ...
      printf("A%u.txt=\"ErrorBloqueo\"",i+1);
      SendDataDisplay();
   }else if(ACiclos[i]==8){
      printf("A%u.txt=\"ErrorApertura\"",i+1);
      SendDataDisplay();
   }else if(ACiclos[i]==9){
      printf("A%u.txt=\"Llenado\"",i+1);
      SendDataDisplay();
   }else if(ACiclos[i]==10){
      printf("A%u.txt=\"PreCale\"",i+1);
      SendDataDisplay();
      ...
      SendDataDisplay();
   }else if(ACiclos[i]==30){
      printf("A%u.txt=\"Empaque\"",i+1);
      SendDataDisplay();
   }
}
   ...
   if(!PruebaEstados){
      if(NivelBajo){
         if(HIG){
            ...
            SensadoNivel=OFF;
            TiempoSensadoNivel=0;
            if(!AlarmaLlenado && !CalderinLleno)   
               Agua_on;
            IniciaLlenado=ON;
         }
      }else{
...
void desbloqueaPuerta(){   
   if(PresionCamara<=2 && !SWBLO){
      if(tDesbloqueo<10){
         flagDesbloqueo=ON;
         conteo++;
         Envio_I2C(0XC0,0,conteo);
...
void abrePuerta(){
   if(PresionCamara<=2 && !SWBL && !DOORO && SWBLO){
      if(tiempoApertura>=50){
         detienePuerta();
         AlarmaApertura=ON;
         flagApertura=OFF;
      }else{
         if(DOOR && tiempoApertura<=2){
            abrio=OFF;
            flagApertura=ON;
            conteo++;
            Envio_I2C(0XC0,0,conteo);
