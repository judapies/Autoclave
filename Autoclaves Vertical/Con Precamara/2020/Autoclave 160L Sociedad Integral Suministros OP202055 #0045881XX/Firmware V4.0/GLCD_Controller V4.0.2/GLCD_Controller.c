...
short AlarmaVacio=OFF,UP=OFF,DOWN=OFF,RIGHT=OFF,LEFT=OFF,flagAbreSecado=0,AlarmaApertura=OFF,flagApertura=OFF,AlarmaBloqueo=OFF,flagBloqueo=OFF,flagDesbloqueo=OFF;
...
//Tiempos//---------------------
int16 TiempoCiclo=0,TiempoLlenado=0,TiempoSensadoNivel=0,Tp=0,Tc=0,tBombaAgua=0,tBombaAguaOff=0,NivelSeleccionado=0,tiempoApertura=0,tBloqueo=0,tDesbloqueo=0;
int8 sEst=0,mEst=0,sSec=0,mSec=0,sEstp=0,mEstp=0,sSecp=0,mSecp=0,j=0,cicloVisto=1,codigoUsuario=0,cicloVisualizado=1,codigoUsuario2=0,codigoUsuarioant=0,NivelDes=0,conteo=0,TTrampa=0;
...
int8 Integral=0,Derivativo=0,CicloLibre=0,difPresionC=0,difPresionPC=0;
...
const char Texto[12][24]={"TEXTIL & INSTRUMENTAL","CAUCHO","LIQUIDOS A","LIQUIDOS B","NO ENVUELTO","ENVUELTO 121",
"ENVUELTO DOBLE","LIQUIDOS C","SOLIIDOS","DESINFECCION","TEST DE COMPONENTES","HORA Y FECHA"};
   ...
   if(Menu==1){ //Menu Principal.
      ...
      if(RX_Buffer[4]==0x01 && RX_Buffer[5]==0x0f && estados[19+RX_Buffer[4]]){//01, Textil & Instrumental
         ...
         if(RX_Buffer[4]==0x01 && RX_Buffer[5]==0x0f && estados[19+RX_Buffer[4]]){//01, Textil & Instrumental
            ...
            SendDataDisplay();
            ConfiguraCiclo(0,15,0,15,0,1,121,7,25,0);
            Op=OFF;
            Op2=OFF;
         }
      }
      ...
      if(RX_Buffer[4]==0x07 && RX_Buffer[5]==0x0f && estados[19+RX_Buffer[4]]){//07, Envuelto Doble 2
         ...
         if(RX_Buffer[4]==0x07 && RX_Buffer[5]==0x0f && estados[19+RX_Buffer[4]]){
            printf("page Funcionamiento");
            SendDataDisplay();       
            ConfiguraCiclo(0,30,0,30,0,7,121,7,25,0);  
            Op=OFF;
            Op2=OFF;
         }
      }
...
//_--------------------------------------------------------------------------------------------------------------------------------------------------------      
      if(Menu==6){ // Menu de Prueba de Componentes
         ...
         if(AlarmaBloqueo){
            printf("t0.txt=\"Bloqueo\"");         
            SendDataDisplay();
         }
         
         if(AlarmaApertura){
            printf("t0.txt=\"Apertura\"");         
            SendDataDisplay();
         }
...
//_--------------------------------------------------------------------------------------------------------------------------------------------------------    
   ...
   if(!DOOR){
      Paso_off;
      if(!Secando && !Finalizo && !Desfogando && !AlarmaEmergencia){
         AlarmaPuerta=ON;
      }
      detieneBloqueo();
   }else{
      AlarmaPuerta=OFF;
      if(!Finalizo && !Secando && !AlarmaEmergencia){
         bloqueaPuerta();
      }
   }
   ...
   if(AlarmaPuerta || AlarmaTermostato || AlarmaSobreTemperatura || AlarmaTiempoProlongado || AlarmaEmergencia || AlarmaPresion || AlarmaBloqueo || AlarmaApertura || AlarmaLlenado){
      ActivaAlarma(1);
      if(AlarmaEmergencia){
         codigoAlarma=1;
      ...
      }else if(AlarmaBloqueo){ // Alarma de Error de Bloqueo
         codigoAlarma=7;
         printf("Funcionamiento.t4f.txt=\" Bloqueo Puerta\"");
         SendDataDisplay();
         printf("Funcionamiento.t5f.txt=\" Cancelado\"");
         SendDataDisplay();
         Desfogue_on;
      ...
      }else if(AlarmaApertura){ // Alarma de Error de Apertura o cierre de puerta
         codigoAlarma=8;
         printf("Funcionamiento.t4f.txt=\" Sistema Puerta\"");
         SendDataDisplay();
         printf("Funcionamiento.t5f.txt=\" Cancelado\"");
         SendDataDisplay();
         Desfogue_on;
