...
short Lectura=0,Expulsa=0,Visua=0,flag_lleno=0;
...
float Leer_Sensor(int media)
{
   ...
   Pt=Pt-0.3; //Ajuste de impedancia de cableado de PT100
...
void ControlaPresion(){
   if(PresionPrecamara16>=20)
   {
      Resistencia_off;
   }
   
   if(PresionPrecamara16<16)
   {
      Resistencia_on;
   }
...
//_--------------------------------------------------------------------------------------------------------------------------------------------------------    
   ...
   {      
      if(EVDesfogue<1 && tiempo_esterilizacion==1 && (Temperatura<100.0 || Temperatura>135.0) && !STOP)
      {
         Alarma=2;
         EV_Agua_off;
         EV_Paso_off;
      ...
      if(BombaVacio==5 && !STOP)
      {
         ControlaPresion();
            ...
            if(PresionCamara16>(Atmosferica-31) && PulsoPositivo==10)
            {
               flag_vacio=1;
               Bomba_on;
               EV_Bomba_on;
