...
short EstadoDesfogue=OFF,EstadoAgua=OFF,EstadoSSR=OFF,PruebaEstados=OFF,disp_Guarda=OFF;
...
short disp_Stop=0;
int8 Control=0;
int8 Menu=0, unidad=1,i=0,Prevacio=10,flagprevacio=10,Calentar=10;
int8 Setpoint=0,Atmosferica=71,fin_ciclo=0,Opcion=1,tiempo_esterilizacion=0,tiempo_secado=0,Exhaust=0;
...
char datoRx[11];
int8 r=0,t=0,r2=0,t2=0,TiempoControl=0,Frio=0,llena=0,PulsoPositivo=10,PulsosVacio=0,flag_pulso=0;
int8 Resistencia=0,MotorPuerta=0,EVEntradaAgua=0,BombaAgua=0;
int8 NivelReservorio=0,Presostato=0,y=0,Chksum=0,flagvacio=10;
int16 t_exhaust=0,tagua=0,treservorio=0,PresionCamara16=0,PresionPreCamara16=0,tvacio=0;
...
// Monitoreo Nivel de Agua//--------------------------------------
   if(!AlarmaLlenado){
      if(!LOW && !Esterilizando && !PruebaEstados && STOP){//Si no sensa agua en el nivel bajo
         AguaIn_on;
         IniciaLlenado=ON;
      }else{
         if(!HIGH && !CalderinLleno && !PruebaEstados && STOP){//Si no sensa agua en el nivel alto y no se ha llenado el calderin
            AguaIn_on;
            IniciaLlenado=ON;
         } else{
            if(TiempoSensadoNivel>=2 && !PruebaEstados)
            {
               SensadoNivel=OFF;
               CalderinLleno=ON;
               IniciaLlenado=OFF;
