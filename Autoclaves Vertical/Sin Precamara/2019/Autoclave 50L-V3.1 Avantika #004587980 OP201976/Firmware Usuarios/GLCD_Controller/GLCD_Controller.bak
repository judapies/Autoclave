...
#define   Agua_off         output_bit(PIN_D6,0)
...
short AlarmaBomba=OFF,guardaCiclo=OFF,Cilindro=OFF,AlarmaLlenado=OFF;
int16 TiempoVacio=600,t_exhaust=0;

//Banderas//--------------------
short IniciaLlenado=OFF,IniciaCiclo=OFF,Esterilizando=OFF,Secando=OFF,SensadoNivel=OFF,Desfogando=OFF,DesfogueSuave=OFF,CalderinLleno=OFF;
...
float Temporal=0.0;
float K=0.006429;
float R7=3000,R8=3100,R9=3100,R10=6800,AN=0,BN=0;
float X[2]={0.0,0.0};
float promediopresion=0.0,promediopresionp=0.0,Aumento=0.0;
int8 h=0,l=0,MediaMovil=20,PantallaPrincipal=0,TipoClave=0,Modulo=0,TTrampa=0;
...
int16 CantidadPulsos=4;
int8 Setpoint=0,Opcion=1,Opcion2=1,tiempo_esterilizacion=0,tiempo_secado=0,tiempoImpresion=0;
int16 tiempos=0,tiempos2=0,tiempos3=0,tiempoctrl=0,Nivel=0,tinicio=0,PresionCamara=0,PresionPreCamara=0,Ciclos=0,tbomba=0,tvacio=0;
...
int16 tbuzzer=0,tsilencio=0,NivelSeleccionado=0,TiempoLlenado=0;
...
void main()
{
   ...
   TiempoVacio=360; 
   Sensor[0].Gain=105.51;
   Sensor[1].Gain=105.51;
      ...
      if(RX_Buffer[3]==0x0f){//0f, recibe caracteres ingresados desde el Display
         ...
         if(TipoClave==0){
            if(RX_Buffer[4]==0x33&&RX_Buffer[5]==0x38&&RX_Buffer[6]==0x39&&RX_Buffer[7]==0x32 &&RX_Buffer[8]==0x00 &&RX_Buffer[9]==0xff){ // Si Ingresa clave para reset general del sistema.
               write_eeprom(300,0);delay_ms(20);write_eeprom(301,0);delay_ms(20);// Reestablece a contraseña de Fabrica y reinicia Programa.
               write_eeprom(302,0);delay_ms(20);write_eeprom(303,0);delay_ms(20);
               write_eeprom(336,0);delay_ms(20);write_eeprom(337,0);delay_ms(20);// Reestablece a contraseña de Fabrica y reinicia Programa.
               write_eeprom(338,0);delay_ms(20);write_eeprom(339,0);delay_ms(20);
               reset_cpu();
            }
...
//_--------------------------------------------------------------------------------------------------------------------------------------------------------
   if(Menu==1){ //Menu Principal.
      ...
      if(RX_Buffer[4]==0x01 && RX_Buffer[5]==0x0f && estados[19+RX_Buffer[4]]){//01, Descontaminación
         delay_ms(5);
         if(RX_Buffer[4]==0x01 && RX_Buffer[5]==0x0f && estados[19+RX_Buffer[4]]){//01, Descontaminación
            printf("page Funcionamiento");
            SendDataDisplay();       
            ConfiguraCiclo(0,0,5,3,0,0,0,0,0,1,121);
            Op=OFF;
            Op2=OFF;
            MenuAnt=Menu;
            //CantidadPulsos=7;
      ...
      if(RX_Buffer[4]==0x02 && RX_Buffer[5]==0x0f && estados[19+RX_Buffer[4]]){//02, Solidos
         delay_ms(5);
         if(RX_Buffer[4]==0x02 && RX_Buffer[5]==0x0f && estados[19+RX_Buffer[4]]){
            printf("page Funcionamiento");
            SendDataDisplay();       
            ConfiguraCiclo(0,0,0,2,0,0,5,0,0,2,121);
            Op=OFF;
            Op2=OFF;
            MenuAnt=Menu;
            //CantidadPulsos=7;
      ...
      if(RX_Buffer[4]==0x03  && RX_Buffer[5]==0x0f && estados[19+RX_Buffer[4]]){//03, Liquidos
         delay_ms(5);
         if(RX_Buffer[4]==0x03 && RX_Buffer[5]==0x0f && estados[19+RX_Buffer[4]]){
            printf("page Funcionamiento");
            SendDataDisplay();       
            ConfiguraCiclo(0,0,5,1,0,0,0,0,0,3,121);
            Op=OFF;
            Op2=OFF;
            MenuAnt=Menu;
            CantidadPulsos=7;
      ...
      if(RX_Buffer[4]==0x0a && RX_Buffer[5]==0x0f && estados[19+RX_Buffer[4]]){//0a, Test de Fugas
         ...
         if(RX_Buffer[4]==0x0a && RX_Buffer[5]==0x0f && estados[19+RX_Buffer[4]]){
            printf("page Funcionamiento");
            SendDataDisplay();       
            ConfiguraCiclo(0,0,5,1,0,0,0,0,0,10,0);
            Op=OFF;
            Op2=OFF;
            MenuAnt=Menu;
            CantidadPulsos=1;
...
//_--------------------------------------------------------------------------------------------------------------------------------------------------------
    if(Menu==3){ //Menu de Tiempo de Secado
      ...
      SendDataDisplay();
         ...
         if(RX_Buffer[4]==0x0d ||RX_Buffer[4]==0x0c){//Tecla Arriba Oprimida
            if(Segundos){
               Tsecadop[0]++;
               if(Tsecadop[0]>9){
                  Tsecadop[1]++;
...
//_--------------------------------------------------------------------------------------------------------------------------------------------------------    
   ...
   if(PresionPreCamara>275 || PresionCamara>350){
      AlarmaPresion=ON;
   }
   ...
   if(Temperatura>Setpoint+3.0 && !Fugas){
   ...
   if(Grafica==1){
      printf("add 14,0,%1.0f",Temperatura*0.66);
      SendDataDisplay();
      printf("add 14,1,%1.0f",PresionCamara*0.66);
      SendDataDisplay();
      printf("add 14,2,%1.0f",PresionPreCamara*0.66);
      SendDataDisplay();
      Grafica=0;
   }
      ...
      if(RX_Buffer[4]==0x11 || (RIGHT && LEFT)){//Tecla Regresar
         ...
         if(RX_Buffer[4]==0x11 || (RIGHT && LEFT)){
            if(!GuardaEEPROM){
               ...
               delay_ms(10);
               write_eeprom(32,(int8)Sensor[0].RPT100);
               delay_ms(10);
