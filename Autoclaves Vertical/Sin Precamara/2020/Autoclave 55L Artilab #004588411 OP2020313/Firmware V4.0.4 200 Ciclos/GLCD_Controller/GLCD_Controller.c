...
#define   Vent_on          output_bit(PIN_D5,1)
#define   Vent_off         output_bit(PIN_D5,0)
...
short EstadoDesfogue=OFF,EstadoAgua=OFF,EstadoSSR=OFF,PruebaEstados=OFF,Ciclo=OFF,flagCalentando=OFF,flagEsterilizando=OFF,disp_Guarda=OFF,guardaCiclo=OFF;
...
short flagBuzzer=OFF,flagSilencio=OFF,Finalizo=OFF,AlarmaPresion=OFF,Libre=OFF,purga=OFF,entro=OFF;
...
int8 h=0,l=0,Ajuste=0,Ajusten=0,MediaMovil=20,TTrampa=0,PantallaPrincipal=0,TipoClave=0,Modulo=0,Digito=0,cicloVisto=0,cicloVisualizado=0,posicionDato=0,j=0,codigoAlarma=0,codigoAnt=0;
...
int16 tiempos=0,tiempos2=0,tiempos3=0,tiempoctrl=0,Nivel=0,tinicio=0,PresionCamara=0,PresionPreCamara=0,Ciclos=0,NivelSeleccionado=0,tguarda=0;
         ...
         */
         ...
         if(RX_Buffer[4]==0x21 || LEFT){//Oprimio salir
            ...
            if(RX_Buffer[4]==0x21 || LEFT){
               ...
               SendDataDisplay();
               PantallaPrincipal=0;
               PruebaEstados=OFF;Desfogue_off;Vent_off;SSR_off;
               ApagaSalidas();
               // Falta apagar Todo
            }
         }
   ...
   || AlarmaCiclo){
      ...
      if(flagImprime){
         ...
         if(!CalderinLleno){
            ...
            if(Temperatura<((float)Setpoint-0.9) && !Desfogando && !Finalizo){
               Esterilizando=OFF;
            }
