...
#define   Agua_on          output_bit(PIN_C0,0)
#define   Agua_off         output_bit(PIN_C0,1)

#define   Desfogue_on      output_bit(PIN_C1,0)
#define   Desfogue_off     output_bit(PIN_C1,1)

#define   AireEnf_on       output_bit(PIN_C2,0)
#define   AireEnf_off      output_bit(PIN_C2,1)
...
//Comunicacion//------------------------
int8 SlaveA0Tx[0X20];
int8 dato=0;            //Contendrá la información a enviar o recibir
int8 direccion=0;       //Contendrá la dirección del esclavo en el bus I2C
int8 posicion=0;        //Contendrá la posición en el buffer de memoria del esclavo donde se almacenará o de donde se recuperará el dato
...
short EstadoDesfogue=OFF,EstadoAgua=OFF,EstadoSSR=OFF,PruebaEstados=OFF,Ciclo=OFF,Enfriando=OFF;
short Op=OFF,Op2=OFF,Minutos=OFF,Segundos=OFF,GuardaEEPROM=OFF,EstadoPaso=OFF,EstadoBuzzer=OFF,flagAlarma=OFF,guardaCiclo=OFF,estadoAire=OFF;
short flagBuzzer=OFF,flagSilencio=OFF,Finalizo=OFF,AlarmaPresion=OFF,Libre=OFF,purga=OFF,entro=OFF,LOW=OFF,HIG=OFF,disp_Guarda=OFF,SensadoNivelL=OFF;

//Tiempos//---------------------
int16 TiempoCiclo=0,TiempoLlenado=0,TiempoSensadoNivel=0,Tp=0,Tc=0,tNivelBajo=0,tguarda=0;
...
float X[2]={0.0,0.0};
float Xx[2]={0.0,0.0};
float promediopresion=0.0,Aumento=0.0,promediopresionp=0.0,Aumento2=0.0;
int8 h=0,l=0,Ajuste=0,Ajusten=0,MediaMovil=20,TTrampa=0,PantallaPrincipal=0,TipoClave=0,Modulo=0,Digito=0,yy=0;
...
float Temperatura=0.0,K=0.0012858,Temperatura2=0.0;
...
int8 Control=0,tiemporecta=0,Controlando=0,timpresion=2,CalculoImpresion=0,Inicio=1,Dato_Exitoso=0,Dato2=0,z=0;
...
int16 tiempos=0,tiempos2=0,tiempos3=0,tiempoctrl=0,Nivel=0,tinicio=0,PresionCamara=0,PresionPreCamara=0,Ciclos=0,NivelSeleccionado=0,tstop=0,tterm=0,tdoor=0;
...
int8 Year=18,Mes=9,Dia=13,Hora=0,Minuto=0,Segundo=0,dow=0,Atmosferica=75,Tapertura=0,cicloVisto=0,cicloVisualizado=0,posicionDato=0,j=0,codigoAlarma=0,codigoAnt=0;
...
void main()
{
   ...
   while(true)
   {
      ...
      if(!Esterilizando || Desfogando || Enfriando){
      ...
      if(RX_Buffer[3]==0x0f || UP){//0f, recibe caracteres ingresados desde el Display
         ...
         if(PantallaPrincipal==0){//Si esta en menu principal
            ...
            if(Opcion==9){// Datos Ciclo
               ...
               SendDataDisplay();       
               ConfiguraCiclo(0,30,0,0,1,4,121,0,0,0);
               Op=OFF;
               Op2=OFF;
               MenuAnt=2;
               ...
               printf("page Funcionamiento");
               SendDataDisplay();       
               ConfiguraCiclo(0,15,0,0,1,1,121,0,0,0);               
               Op=OFF;
               Op2=OFF;
               MenuAnt=2;               
               ...
               printf("page Funcionamiento");
               SendDataDisplay();       
               ConfiguraCiclo(0,30,0,3,0,3,121,0,0,0);
               Op=OFF;
               Op2=OFF;
               MenuAnt=Menu;
               ...
               printf("page Funcionamiento");
               SendDataDisplay();       
               ConfiguraCiclo(0,15,0,3,0,2,121,0,0,0);
               Op=OFF;
               Op2=OFF;
               MenuAnt=Menu;
               //if(Temperatura>60)TTrampa=TTrampa+5;
...
//_--------------------------------------------------------------------------------------------------------------------------------------------------------      
   if(Menu==5){//Menu de Nivel
      ...
      if(Nivel==2){
         printf("tnivel.txt=\"Rapido\"");
         SendDataDisplay();
      }else{
         printf("tnivel.txt=\"Lento\"");
         SendDataDisplay();
      }
   }
//_--------------------------------------------------------------------------------------------------------------------------------------------------------      
...
//_--------------------------------------------------------------------------------------------------------------------------------------------------------      
      if(Menu==6){ // Menu de Prueba de Componentes
         ...
         if(RX_Buffer[4]==0x05)//Oprimio EV Aire
         {
         ...
         if(RX_Buffer[4]==0x21 || LEFT){//Oprimio salir
            ...
            if(RX_Buffer[4]==0x21 || LEFT){
               ...
               SendDataDisplay();
               PantallaPrincipal=0;
               PruebaEstados=OFF;Desfogue_off;Agua_off;Aire_off;AireEnf_off;SSR_off;
               ApagaSalidas();
               // Falta apagar Todo
            }
         }
...
//_--------------------------------------------------------------------------------------------------------------------------------------------------------
   if(Menu==8){ //Menu Ciclos Personalizados
      ...
      if(RX_Buffer[4]>0x00 && RX_Buffer[4]<0x04){//Personalizados
         ...
         SendDataDisplay();
         if(CiclosPersonalizados[RX_Buffer[4]-1].NivelDesfogue==1){
            printf("desfogue.txt=\"Lento\"");
            SendDataDisplay();
         }else{
            printf("desfogue.txt=\"Rapido\"");
            SendDataDisplay();
         }
         ...
         SendDataDisplay();
         if(CiclosPersonalizados[CicloLibre-1].NivelDesfogue==1){
            printf("desfogue.txt=\"Lento\"");
            SendDataDisplay();
         }else{
            printf("desfogue.txt=\"Rapido\"");
      ...
      if((RX_Buffer[4]==0x70 || UP)&& CicloLibre>0){//70, Iniciar Ciclo
         ...
         if((RX_Buffer[4]==0x70 || UP)&& CicloLibre>0){
            ...
            if(CiclosPersonalizados[CicloLibre-1].NivelDesfogue!=1){
               ...
               CiclosPersonalizados[CicloLibre-1].Segundosec,CiclosPersonalizados[CicloLibre-1].MinutoSec,0,CicloLibre+4,
               CiclosPersonalizados[CicloLibre-1].Temperatura,0,0
               ,0);
            }else{
               ConfiguraCiclo(CiclosPersonalizados[CicloLibre-1].Segundoest,CiclosPersonalizados[CicloLibre-1].Minutoest,
               CiclosPersonalizados[CicloLibre-1].Segundosec,CiclosPersonalizados[CicloLibre-1].MinutoSec,1,CicloLibre+4,
               CiclosPersonalizados[CicloLibre-1].Temperatura,0,0
               ,0);
...
//_--------------------------------------------------------------------------------------------------------------------------------------------------------    
   ...
   if(PresionCamara>275+Atmosferica){
      AlarmaPresion=ON;
   }
   
   if(Temperatura>105 && PresionCamara<10+Atmosferica && !Desfogando){
      AlarmaCiclo=ON;
   }
   ...
   if(TiempoCiclo>3000){
      ...
      if(PresionCamara<10+Atmosferica ){
         AlarmaTiempoProlongado=ON;
      }
   }else{
   ...
   if(Temperatura>Setpoint+4.0 && !Enfriando){
   ...
   if(!DOOR){
      //Desfogue_off;
      //Solenoide_off;
      if(CalderinLleno && !Secando && !Finalizo && !Desfogando){
         AlarmaPuerta=ON;
      }
   }else{
      AlarmaPuerta=OFF;
   ...
   || AlarmaCiclo){
      ...
      if(flagImprime){
         ...
         if(!CalderinLleno){
            printf("Funcionamiento.t4f.txt=\" Aplique Agua\"");
            SendDataDisplay();
            ...
            if(Temperatura<((float)Setpoint-0.9) && !Desfogando && !Enfriando){
               Esterilizando=OFF;
            }
                  ...
                  if(desfoguelento){
                     Controlando=0;
                        if(Temperatura<=80){    
                           ...
                           Aire_off;
                           AireEnf_off;
                           if(PresionCamara<2+Atmosferica){
                              ActivaAlarma(2);
                              printf("Funcionamiento.t4f.txt=\" Ninguna\"");
                              SendDataDisplay();
                           ...
                           printf("Funcionamiento.t4f.txt=\" Ninguna\"");
                           SendDataDisplay();
                           printf("Funcionamiento.t5f.txt=\" Enfriando\"");
                           SendDataDisplay();
                           Control=0;
                           Controlando=0;
                           SSR_off;
   ...
   {                            
      ...
      printf("fecha.txt=\"%2.1fT:%2.1f Fecha:%02u/%02u/20%02u Hora: %02u:%02u:%02u\"",Aumento2,Tactual,Dia,Mes,Year,Hora,Minuto,Segundo);
      SendDataDisplay();
   }
...
//_--------------------------------------------------------------------------------------------------------------------------------------------------------
      ...
      if(Opcion2==1){
         ...
         printf("Config.t2c.txt=\"T. Trampa\"");
         SendDataDisplay();      
         printf("t3c.txt=\"%03u\"",TTrampa);
         SendDataDisplay();
         if(RX_Buffer[4]==0x0a){//Tecla Arriba Oprimida
               if(TTrampa<100)
                  ++TTrampa;
