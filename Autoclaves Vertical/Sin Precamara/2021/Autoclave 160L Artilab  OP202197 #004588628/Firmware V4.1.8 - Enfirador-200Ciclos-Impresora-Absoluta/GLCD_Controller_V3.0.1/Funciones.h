...
//Lectura EEPROM//--------------------------
   ...
   delay_ms(10);
   pulsosPurgaProgramados=read_eeprom(47);
...
void LimitaValores(){
   ...
   if(pulsosPurgaProgramados>30 || pulsosPurgaProgramados<1){
      pulsosPurgaProgramados=15;
      write_eeprom(47,15);
      delay_ms(10);
   }
   if(sg1==10){
...
void purgaAire(){
   if(PresionCamara>(int16)Atmosferica+30 && !purgo){
      purgaAire2=ON;
   }else{
      purgaAire2=OFF;
      if(!purgo){
   ...
   if(purgaAire2){
      ...
      if(pulsosPurga>=pulsosPurgaProgramados){
         purgaAire2=OFF;
         purgo=ON;
      }
   }
