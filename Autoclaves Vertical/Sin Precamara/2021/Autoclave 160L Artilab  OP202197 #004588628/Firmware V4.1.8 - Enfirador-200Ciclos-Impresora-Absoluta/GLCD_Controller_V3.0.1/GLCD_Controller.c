...
#ifdef Enfriador
#define   Enfr_on          output_bit(PIN_C0,0)
#define   Enfr_off         output_bit(PIN_C0,1)
#else
...
//Banderas//--------------------
short IniciaLlenado=OFF,CalderinLleno=OFF,IniciaCiclo=OFF,Esterilizando=OFF,Secando=OFF,SensadoNivel=OFF,Desfogando=OFF,DesfogueSuave=OFF,purgo=OFF,purgaAire2=OFF;
...
short flagDesfogando=OFF,Op=OFF,Op2=OFF,Minutos=OFF,Segundos=OFF,GuardaEEPROM=OFF,EstadoPaso=OFF,EstadoBuzzer=OFF,flagAlarma=OFF,borra=OFF,LOW=OFF,HIG=OFF;
...
int8 Integral=0,Derivativo=0,CicloLibre=0,pulsosPurga=0,pulsosPurgaProgramados=0;
...
int16 tiempos=0,tiempos2=0,tiempos3=0,tiempoctrl=0,Nivel=0,tinicio=0,PresionCamara=0,PresionPreCamara=0,Ciclos=0,NivelSeleccionado=0,tBombaAgua=0,tBombaAguaOff=0,tPurgaAire=0,posicionDato=0;
signed int  Contrasena[4]={0,0,0,0};      // Contrasena de 4 digitos
signed int  Password[4]={0,0,0,0};        // Contrasena Almacenada de 4 digitos
int8 r=0,t=0,r22=0,t2=0,TiempoControl=0;
int8 y=0,ConteoBuzzer=0;
int16 t_exhaust=0,tbuzzer=0,tsilencio=0,tiempoAgua=0,tStop=0,tTermostato=0,cicloVisto=0,cicloVisualizado=0,i=0,j=0,tright=0;
...
//_--------------------------------------------------------------------------------------------------------------------------------------------------------    
   ...
   if(!DOOR){
      ...
      #else
         if(!Secando && !Finalizo && !Desfogando && CalderinLleno){
            AlarmaPuerta=ON;
         }
      #endif
   }else{
...
//_--------------------------------------------------------------------------------------------------------------------------------------------------------
      ...
      if(Opcion2==1){
         ...
         printf("Config.t2c.txt=\"Pulsos Purga\"");
         SendDataDisplay();      
         printf("t3c.txt=\"%02u\"",pulsosPurgaProgramados);
         SendDataDisplay();
         if(RX_Buffer[4]==0x0a || UP){//Tecla Arriba Oprimida
            delay_ms(20);
            if(RX_Buffer[4]==0x0a || UP){
               if(pulsosPurgaProgramados<30)
                  pulsosPurgaProgramados+=1;
               RX_Buffer[4]=0x00;  
               RX_Buffer2[4]=0x00;            
               //BorraBuffer();
            }
      ...
      if(RX_Buffer[4]==0x11 || (RIGHT && LEFT)){//Tecla Regresar
         ...
         if(RX_Buffer[4]==0x11 || (RIGHT && LEFT)){
            if(!GuardaEEPROM){
               ...
               write_eeprom(46,Atmosferica);
               delay_ms(10);
               write_eeprom(47,pulsosPurgaProgramados);
               delay_ms(10);
               GuardaEEPROM=ON;
            }
            printf("page MenuPrincipal");
