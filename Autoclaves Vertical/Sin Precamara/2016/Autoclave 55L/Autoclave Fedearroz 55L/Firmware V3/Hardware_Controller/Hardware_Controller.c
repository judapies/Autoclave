...
#define   EV_in_Agua_on      output_bit(PIN_B2,1)
#define   EV_in_Agua_off     output_bit(PIN_B2,0)
...
void main()
{
   ...
   EV_Desfogue_off;Resistencia_off;
   usb_init();
   usb_task();
...
//_--------------------------------------------------------------------------------------------------------------------------------------------------------    
   ...
   {      
      ...
      if(Control==1 && !DOOR && !STOP && AlarmaAgua==0 && Alarma==0) // Si esta en condiciones normales para ejecutar el control.
      {
         ...
         {  
              // Electrovalvula que da paso de la precamara a la Camara.
